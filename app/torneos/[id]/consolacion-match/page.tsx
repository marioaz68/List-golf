...
import ConsolationMatchView from "./ConsolationMatchView";
...
export async function generateMetadata(props: {
  ...
  const { data: t } = await admin
    ...
    .maybeSingle();
  return {
    title: `Consolación Match Play · ${t?.name ?? "Torneo"}`,
  };
}

export default async function ConsolacionMatchPage(props: {
  params: Promise<RouteParams> | RouteParams;
}) {
  const params = await Promise.resolve(props.params);
  ...
  return (
    <main className="min-h-dvh bg-gradient-to-br from-[#020617] via-[#0b132b] to-[#0a1220] p-3 text-white sm:p-5">
      <ConsolationMatchView tournamentId={tournamentId} />
    </main>
  );
}
