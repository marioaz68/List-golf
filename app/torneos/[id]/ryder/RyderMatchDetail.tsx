...
type MatchDetail = {
  ...
  top_label: string;
  bottom_label: string;
  top_players: [PlayerInfo, PlayerInfo | null] | PlayerInfo[];
  bottom_players: [PlayerInfo, PlayerInfo | null] | PlayerInfo[];
  pair_format: string;
...
type Props = {
  ...
  tournamentId: string;
  matchId: string | null;
  /** Etiqueta corta del encabezado (ej. "Grupo 10 · Individual"). */
  headerLabel?: string;
};
...
function activePlayers(
  players: MatchDetail["top_players"] | undefined,
  singles: boolean
): PlayerInfo[] {
  ...
  const real = list.filter(
    (p) => p.label && p.label !== "Jugador B" && p.label.trim() !== ""
  );
  if (singles) return (real.length ? real : list).slice(0, 1);
  return (real.length ? real : list).slice(0, 2);
}

export default function RyderMatchDetail({
  open,
  onClose,
  tournamentId,
  matchId,
  ...
  const isSingles = detail?.scoring_format === "singles";
  const topPlayers = activePlayers(detail?.top_players, Boolean(isSingles));
  const bottomPlayers = activePlayers(
    detail?.bottom_players,
    Boolean(isSingles)
  );
  const topName = detail?.top_label ?? "Socios";
  const bottomName = detail?.bottom_label ?? "Caddies";
  ...
  return (
    ...
    >
      <div
        className="max-h-[92dvh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-emerald-600/40 bg-emerald-950 text-white shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="sticky top-0 z-10 flex items-start justify-between gap-2 border-b border-emerald-600/40 bg-emerald-950/95 px-4 py-3 backdrop-blur">
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-[0.24em] text-emerald-300/80">
              {headerLabel ??
                (detail
                  ? `${formatHint} · pos ${detail.position_no}`
                  : "Detalle del match")}
            </div>
            <h2 className="mt-0.5 truncate text-lg font-extrabold">
              {topName} <span className="text-emerald-500">vs</span> {bottomName}
            </h2>
            {detail?.result_text ? (
            ...
            {detail ? (
              <p className="mt-0.5 text-[11px] text-emerald-200/70">
                ...
                <span className="text-emerald-200/50"> · {formatHint}</span>
              </p>
            ) : null}
          </div>
          <button
            type="button"
            className="rounded border border-emerald-600/40 bg-emerald-900/60 px-2 py-1 text-[11px] font-semibold hover:bg-emerald-800/60"
            onClick={onClose}
            aria-label="Cerrar"
          >
            ✕
        ...
        <div className="space-y-4 px-3 py-3 sm:px-4">
          {loading ? (
            <div className="rounded border border-emerald-600/40 bg-emerald-900/60 p-3 text-sm text-emerald-200/80">
              Cargando detalle…
            </div>
          ) : null}
          {error ? (
...
function PlayoffTable({
  ...
  const role = (
    ...
    if (!h.breakdown?.nets) return null;
...
function ScoreboardSide({
  ...
  const color = tone === "top" ? "border-white/40 bg-white/10" : "border-amber-400/40 bg-amber-950/30";
  const dot = tone === "top" ? "bg-white" : "bg-amber-400";
  return (
...
function PointsChart({
  ...
  return (
    <div className="rounded-lg border border-emerald-600/40 bg-emerald-900/60 p-2">
      ...
      <div className="mt-1 flex flex-wrap items-center justify-center gap-3 text-[10px] text-emerald-200/80">
        <span className="inline-flex items-center gap-1">
          <span className="h-2 w-3 rounded-sm bg-white" /> {topLabel} arriba
        </span>
        <span className="inline-flex items-center gap-1">
          <span className="h-2 w-3 rounded-sm bg-amber-400" /> {bottomLabel}{" "}
          arriba
        </span>
        <span className="text-emerald-200/50">Línea AS = empate acumulado</span>
        {decidedAtHole != null ? (
...
function HoleTable({
  ...
  function netOf(h: HoleDetail, who: "tA" | "tB" | "bA" | "bB"): number | null {
    if (isSingles) {
      if (who === "tA") return h.breakdown?.top?.net ?? h.top_player_a_strokes;
      if (who === "bA")
        return h.breakdown?.bottom?.net ?? h.bottom_player_a_strokes;
      return null;
    }
    const nets = h.breakdown?.nets;
    if (nets) {
  ...
  function strokesReceivedOnHole(
    ...
    if (isSingles) {
      if (who === "tA") return h.breakdown?.top?.strokes_received ?? 0;
      if (who === "bA") return h.breakdown?.bottom?.strokes_received ?? 0;
      return 0;
    }
    const sr = h.breakdown?.strokes_received;
    if (!sr) return 0;
  ...
  /** En low_high: "low"/"high" por pareja. En singles no aplica. */
  function role(
    h: HoleDetail,
    who: "tA" | "tB" | "bA" | "bB"
  ): "low" | "high" | null {
    if (isSingles) return null;
    if (!h.breakdown?.nets) return null;
    const { nets } = h.breakdown;
    if (who === "tA") return nets.top_a <= nets.top_b ? "low" : "high";
    if (who === "tB") return nets.top_b < nets.top_a ? "low" : "high";
  ...
  const stickyName =
    "sticky left-0 z-30 border-b border-r border-white/10 px-2 py-1 text-left text-[10px] font-semibold leading-tight shadow-[6px_0_12px_-4px_rgba(0,0,0,0.55)] w-[120px] min-w-[120px] max-w-[140px]";
  const holeTh =
    "w-[24px] min-w-[24px] border-b border-white/10 px-0 py-0.5 text-center text-[9px] font-bold text-emerald-50 sm:w-7";
  const subTh =
    "w-[30px] min-w-[28px] border-b border-l border-white/10 px-0 py-0.5 text-center text-[9px] font-bold text-white sm:w-[34px]";
  const cellTd =
  ...
  return (
    <div className="horizontal-scroll overflow-x-auto rounded-lg border border-emerald-600/40 bg-emerald-900/60">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-emerald-600/40 bg-emerald-950/80 px-3 py-1.5 text-[9px] text-emerald-200/70">
        <span>
      ...
      <table className="min-w-full border-separate border-spacing-0 text-[10px] text-white">
        <thead>
          <tr className="bg-gradient-to-r from-emerald-950 via-emerald-900 to-emerald-950 text-emerald-50">
            <th
            ...
            <th className={subTh}>
              OUT
              {hasPar ? (
                <span className="block text-[7px] font-normal text-emerald-300/70">
                  neto vs par
                </span>
              ) : null}
            </th>
        ...
        <tbody>
          ...
          {playerRows.map((p, idx) => {
            const stripe =
              idx % 2 === 0 ? "bg-emerald-900/50" : "bg-emerald-900/30";
            const stripeHex = idx % 2 === 0 ? "#064e3b" : "#065f46";
            const accent =
              p.team === "top"
                ? "border-l-2 border-l-white/50"
                : "border-l-2 border-l-amber-400/50";
          ...
          {(() => {
            const stripe = "bg-emerald-950";
            const stripeHex = "#064e3b";
            const d9 = diffAt(9);
            const d18 = diffAt(18);
            const dIn =
              d9 != null && d18 != null ? d18 - d9 : null;
            return (
              <tr className={`border-t-2 border-emerald-500/30 ${stripe}`}>
                <td
                  className={`${stickyName} border-l-2 border-l-amber-400/60`}
                  style={{ backgroundColor: stripeHex }}
                >
      ...
      <div className="space-y-1 border-t border-white/10 px-2 py-1.5 text-[9px] text-slate-400">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          {!isSingles ? (
            ...
            <span className="inline-flex items-center gap-1">
              1 punto al neto menor del hoyo
            </span>
          )}
          <span className="inline-flex items-center gap-1">
