...
function renderLevelDot(level: string | null) {
  if (level === "advanced") {
    return <span style={dotBlue} title="Advanced" />;
  }

  if (level === "intermediate") {
    return <span style={dotRed} title="Intermediate" />;
  }

  if (level === "beginner") {
    return <span style={dotGreen} title="Beginner" />;
  }
                      ...
                      {row.currentCaddie ? (
                        <div style={{ display: "grid", gap: 4 }}>
                          ...
                          <div style={{ fontSize: 11, color: "#64748b" }}>
                            {displayCaddieName(row.currentCaddie)}
                          </div>
                        </div>
                      ) : (
                        "Sin asignar"
