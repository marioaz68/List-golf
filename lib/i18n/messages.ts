import type { Locale } from "./locale";

const es = {
  language: {
    label: "Idioma",
    es: "ES",
    en: "EN",
    ariaEsActive: "Idioma actual: español",
    ariaEsInactive: "Cambiar la interfaz a español",
    ariaEnActive: "Idioma actual: inglés",
    ariaEnInactive: "Cambiar la interfaz a inglés",
  },
  nav: {
    enter: "Entrar",
    signOut: "Salir",
    signOutTitle: "Cerrar sesión para entrar con otro usuario",
  },
  sidebar: {
    activeTournament: "Torneo activo",
    loading: "Cargando...",
    noName: "Sin nombre",
    noTournament: "Sin torneo seleccionado",
    modePrefix: "Modo",
    operation: "Operación",
    configShort: "Config.",
    configuration: "Configuración",
    setupSection: "Configuración del torneo",
    publicPage: "Página pública",
    switchTo: "Cambiar a",
    listTournaments: "Listado de torneos",
    brand: "List.golf",
    selectTournamentHint: "Selecciona un torneo para ver este menú.",
    openMenu: "Abrir menú de navegación",
    closeMenu: "Cerrar menú",
    nav: {
      dashboard: "Panel principal",
      players: "Jugadores",
      entries: "Inscripciones",
      auction: "Subasta",
      teeSheet: "Salidas/Grupos",
      scoreEntry: "Captura de tarjetas",
      scorecards: "Tarjetas",
      scorecardsMp: "Tarjetas MP (imprimir)",
      leaderboard: "Clasificación",
      cercanos: "Más cerca (pares 3)",
      cercanosPremios: "Premios pares 3",
      caddies: "Caddies",
      ritmo: "Ritmo del campo",
      seguimientoCaptura: "Capturas retrasadas",
      reports: "Reportes",
      tournaments: "Torneos",
      editTournament: "Editar torneo",
      tournamentSetup: "Setup torneo",
      clubs: "Clubes",
      courses: "Campos",
      courseHolePoints: "Puntos yardas",
      categories: "Categorías",
      rounds: "Rondas",
      teeSets: "Sets de salida",
      tournamentHoles: "Hoyos torneo",
      cutRules: "Reglas de corte",
      competitionRules: "Liga",
      prizeRules: "Premios",
      convocatoria: "Convocatoria",
      categoryTemplates: "Plantillas cat.",
      users: "Usuarios",
      handicapCommittee: "Comité de Handicap",
      committeeVote: "Votación",
      committeeSelection: "Selección de jugadores",
      committeeGhinData: "Datos GHIN",
      capturaTelegram: "Modo Telegram",
      fbAdmin: "Restaurante",
      fbCocina: "Cocina · Pedidos",
      fbCuentas: "Cuentas · Cobros",
      fbNuevoPedido: "Nuevo pedido (verbal)",
      fbReportes: "Reportes del día",
      fbDisputas: "Disputas (comité)",
      fbMesero: "Mesero · Restaurante",
      fbInventario: "Inventario",
      fbFraccionamiento: "Fraccionamiento",
      fbCuentasDeposito: "Cuentas de depósito",
      fbRestaurante: "Perfil restaurante",
      dailyRounds: "Rondas diarias",
    },
  },
  common: {
    tournament: "TORNEO",
    change: "Cambiar",
    home: "← Inicio",
    noCategory: "Sin categoría",
  },
  publicHome: {
    title: "Torneos públicos",
    subtitle: "Posters y acceso a la página pública de cada torneo.",
    labelScope: "Alcance",
    labelCity: "Ciudad",
    labelClub: "Club",
    scopeAll: "Todos",
    scopeActive: "Torneos activos",
    scopeFuture: "Torneos futuros",
    scopeHistoric: "Torneos históricos",
    cityAll: "Todas las ciudades",
    cityUnknown: "Sin ciudad",
    clubAll: "Todos los clubs",
    apply: "Aplicar",
    clear: "Limpiar",
    countNone: "0 torneos",
    countOne: "1 torneo",
    countMany: "{n} torneos",
    empty: "No hay torneos con estos filtros.",
    noPoster: "Sin poster",
    noClub: "Sin club",
    statusTbd: "Por definir",
    statusUpcoming: "Próximo",
    statusActive: "Activo",
    statusHistoric: "Histórico",
    scopeHint:
      "Activos: ya iniciaron y aún no cierran (si hay fecha de cierre, hasta ese día; si no, hasta 4 semanas después del inicio). Futuros: inicio posterior a hoy. Históricos: ya cerraron (cierre anterior a hoy; sin cierre, pasadas 4 semanas desde el inicio).",
    loadErrorTitle: "No pudimos cargar el listado de torneos",
    loadErrorBody:
      "Puede deberse a permisos en Supabase (RLS), a que falte una columna en la base de producción respecto al código (migraciones no aplicadas) o a otro error de PostgREST. Revisa el detalle técnico abajo y los runtime logs en Vercel.",
    loadErrorTechnical: "Detalle del servidor",
  },
  dashboard: {
    title: "Panel principal",
    kpiActive: "Torneos activos",
    kpiPlayers: "Jugadores totales",
    kpiLive: "Tarjetas en vivo",
  },
  reports: {
    title: "Reportes",
  },
  players: {
    title: "Jugadores",
  },
  leaderboard: {
    title: "Clasificación",
  },
  entries: {
    title: "Inscripciones",
    list: {
      heading: "Jugadores inscritos",
      searchPlaceholder: "#, nombre, club, estatus o ronda...",
      optionClub: "Club",
      optionCat: "Cat",
      thNumber: "#",
      thPlayer: "Jugador",
      thClub: "Club",
      thHcp: "Hcp",
      thHcp80: "HCP 80%",
      thHcp80Title:
        "Handicap de torneo oficial al 80%: HI × (Slope/113) + (CR − Par), luego × 0.80. Muestra HI, salida, slope, CR, par y CH.",
      thCat: "Cat",
      thStatus: "Estatus",
      thSignatures: "Firmas",
      thActions: "Acciones",
      statusActive: "Activo",
      statusWithdrawn: "Baja",
      statusDQ: "DQ",
      noResults: "Sin resultados",
      btnSignatures: "FIRMAS",
      btnDelete: "ELIMINAR",
      deletePending: "Eliminando...",
      confirmDelete:
        "¿Eliminar definitivamente? Se eliminará si no tiene hoyos capturados.",
      btnRea: "REA",
      restorePending: "Restaurando...",
      confirmRestoreWithdrawn: "¿Reactivar este jugador en el torneo?",
      confirmRestoreDq: "¿Quitar DQ y regresar el jugador a activo?",
      btnWithdraw: "Baja",
      withdrawPending: "Dando de baja...",
      confirmWithdraw: "¿Dar de baja a este jugador del torneo?",
      btnDq: "DQ",
      dqPending: "Marcando DQ...",
      confirmDq:
        "¿Marcar DQ? Esto pondrá 400 a la ronda actual/última del torneo y dejará al jugador como DQ.",
      alertNoRoundId: "No se encontró round_id en la URL.",
      linksCopied: "Ligas copiadas al portapapeles",
      linksError: "Error generando ligas",
      linksPlayer: "Jugador:",
      linksMarker: "Marcador:",
      linksWitness: "Testigo:",
      roundSigTitle: "R{round}: {count} firma(s)",
      roundBallCaptured: "R{round}: capturada",
      roundBallCapturedOpen: "R{round}: capturada, sin cerrar",
      roundBallPending: "R{round}: sin capturar",
      roundBallClosed: "R{round}: cerrada",
      roundBallLegend:
        "Rojo: sin capturar · Ámbar: capturada sin cerrar · Verde: tarjeta cerrada",
      btnTelegramKit: "KIT",
      btnTelegramKitTitle: "Guía Telegram para este jugador",
    },
    telegramKit: {
      title: "Kit de conexión Telegram",
      backToEntries: "← Volver a inscritos",
      playerLabel: "Jugador",
      tournamentLabel: "Torneo",
      botUsernameLabel: "Bot de Telegram del torneo",
      botUsernameHint:
        "Dile al jugador que busque este usuario en Telegram, pulse Iniciar y escriba HOLA. Copia el @ para dictárselo.",
      copyBotUsername: "Copiar @usuario",
      copied: "Copiado",
      staffScriptTitle: "Qué decirle al jugador",
      staffScript:
        "«Abre Telegram, busca @{bot}, pulsa Iniciar y escribe HOLA. Si el bot te reconoce, ya estamos enlazados; si no, copia tu ID numérico y se lo damos al comité en esta pantalla.»",
      phaseConnect: "Paso 1 — Conectar Telegram",
      phaseVerify: "Paso 2 — Verificar enlace",
      phaseDeliver: "Paso 3 — Entregar kit",
      statusLinked: "Telegram vinculado en la ficha (user ID guardado).",
      statusNotLinked: "Aún no hay Telegram vinculado.",
      statusKitSent: "Kit enviado por Telegram; esperando que el jugador responda RECIBIDO.",
      statusKitReceived: "El jugador confirmó recibido del kit.",
      step1Title: "1. Abrir el bot en Telegram",
      step1BodyConfigured:
        "El jugador debe abrir el bot con el usuario de arriba, pulsar «Iniciar» y escribir HOLA.",
      step1BodyNotConfigured:
        "Configura NEXT_PUBLIC_TELEGRAM_BOT_USERNAME en el servidor (ej. @ListGolfBot).",
      openBotLink: "Abrir bot en Telegram",
      step2Title: "2. Obtener el ID si el bot no reconoce al jugador",
      step2Body:
        "Pide que escriba HOLA o ID al bot @listgolf. El bot responderá con un número entre líneas ━━━━. También puedes usar @userinfobot. Abajo aparecen los últimos que escribieron al bot sin vincular.",
      pendingLinksTitle: "Últimos mensajes al bot (sin vincular)",
      pendingLinksHint:
        "Si el jugador ya escribió HOLA, busca su nombre aquí y pulsa «Usar este ID».",
      pendingLinksEmpty:
        "Nadie sin vincular ha escrito al bot recientemente. Que el jugador envíe HOLA o ID a @listgolf.",
      pendingLinksUseId: "Usar este ID",
      openUserinfobot: "Abrir @userinfobot",
      step3Title: "3. Guardar ID en la ficha",
      step3Body:
        "Guarda el ID para que el bot identifique a este jugador en adelante.",
      verifyTitle: "Verificar que el enlace funciona",
      verifyBody:
        "Envía un mensaje de prueba al Telegram del jugador. Si lo recibe, el vínculo es correcto.",
      btnVerify: "Enviar verificación por Telegram",
      verified: "Mensaje de verificación enviado.",
      deliverTitle: "Entregar kit del torneo",
      deliverBody:
        "Envía por Telegram el contenido del kit y pide al jugador que responda RECIBIDO cuando lo haya leído.",
      kitPreviewLabel: "Vista previa del mensaje",
      kitExtraNote: "Nota adicional para el jugador (opcional)",
      btnDeliverKit: "Enviar kit por Telegram",
      kitSent: "Kit enviado. Espera que el jugador escriba RECIBIDO al bot.",
      formTitle: "Registrar ID en la ficha del jugador",
      fieldUserId: "Telegram user ID (solo números)",
      fieldChatId: "Chat ID (opcional)",
      fieldChatHint:
        "Suele coincidir con el user id en chats privados; déjalo vacío si no lo necesitas.",
      btnSave: "Guardar en ficha",
      btnClear: "Quitar vínculo Telegram",
      saving: "Guardando…",
      missingParams: "Faltan torneo o jugador en la liga.",
      notEnrolled: "Este jugador no está inscrito en el torneo indicado.",
      invalidUserId: "El user ID debe contener solo dígitos.",
      saveError: "No se pudo guardar",
      saved: "Guardado correctamente.",
      partialDeliveryLabel: "Entrega parcial (el jugador aún debe recibir algo)",
      partialPendingPlaceholder:
        "Ej. credencial, gorra, reglamento impreso…",
      partialPendingHint:
        "Si marcas esto, el jugador podrá responder RECIBIDO PARCIAL y luego RECIBIDO cuando tenga todo.",
      statusKitPartial: "Recibió el kit de forma parcial.",
      linkKitContent: "Editar contenido del kit del torneo",
      statusKitPendingItems: "Pendiente de entregar:",
    },
    telegramKitContent: {
      title: "Contenido del kit Telegram",
      backEntries: "← Inscripciones",
      tournament: "Torneo",
      loadTournament: "Cargar",
      editing: "Editando",
      greeting: "Saludo",
      body: "Cuerpo del mensaje",
      footer: "Instrucciones finales (RECIBIDO / PARCIAL)",
      placeholders:
        "Variables: {player_name}, {tournament_name}. Un renglón por línea en el cuerpo.",
      save: "Guardar plantilla",
      saved: "Plantilla guardada.",
      noTournaments: "No hay torneos.",
    },
    bulk: {
      title: "Inscripción masiva",
      searchPlaceholder: "Buscar nombre o club...",
      allClubs: "Todos los clubs",
      allCats: "Todas cat.",
      selectVisibleBusy: "Seleccionando...",
      selectVisible: "Seleccionar visibles",
      clearVisibleBusy: "Limpiando...",
      clearVisible: "Limpiar visibles",
      visibleCount: "Visibles:",
      selectedCount: "Seleccionados:",
      selectedVisibleCount: "Seleccionados visibles:",
      thSel: "Sel",
      thPlayer: "Jugador",
      thClub: "Club",
      thHi: "HI",
      thCat: "Cat",
      emptyPlayers: "Sin jugadores para mostrar",
      enrolling: "Inscribiendo...",
      enrollSelected: "Inscribir seleccionados ({n})",
    },
    summary: {
      title: "Resumen del torneo",
      players: "Jugadores:",
      categories: "Categorías:",
      clubs: "Clubs:",
      byCategory: "Jugadores por categoría",
      thCategory: "Categoría",
      thEnrolled: "Inscritos",
      thQuota: "Cupo",
      thState: "Estado",
      full: "LLENA",
      spotsFree: "{n} libres",
      noLimit: "Sin límite",
      noData: "Sin datos",
      byClub: "Jugadores por club",
      thClub: "Club",
      thPlayers: "Jugadores",
      noClub: "Sin club",
    },
    single: {
      title: "Inscribir jugador",
      searchPlaceholder: "Buscar nombre o club...",
      newPlayer: "Nuevo jugador",
      thPlayer: "Jugador",
      thClub: "Club",
      thHi: "HI",
      thAge: "Edad",
      thCategory: "Categoría",
      thAction: "Acción",
      categoryNormal: "Categoría normal",
      unnamed: "Sin nombre",
      auto: "Auto",
      noHi: "Sin HI",
      enrolling: "Inscribiendo...",
      enroll: "Inscribir",
      emptySearch: "No se encontró jugador en la lista general.",
    },
    excel: {
      import: "Importar del Excel",
    },
    bulkSelector: {
      title: "Inscripción masiva de jugadores",
      searchPlaceholder: "Buscar jugador...",
      allCategories: "Todas categorías",
      allClubs: "Todos los clubes",
      selectVisible: "Seleccionar visibles",
      thSel: "Sel",
      thPlayer: "Jugador",
      thClub: "Club",
      thHi: "HI",
      thCategory: "Categoría",
      enrollSelected: "Inscribir seleccionados ({n})",
    },
    page: {
      tournamentLabel: "Torneo",
      registrationBlocked:
        "La inscripción, edición de categoría/handicap y eliminación de jugadores está bloqueada.",
      registrationOpen:
        "Puedes inscribir jugadores, editar categoría/handicap y eliminar inscritos.",
      closeRegistration: "Cerrar inscripciones",
      bulkClosedNote:
        "Inscripciones cerradas. Reabre el torneo para usar inscripción masiva.",
      load: "Cargar",
      registrationClosedTitle: "Inscripciones cerradas",
      registrationOpenTitle: "Inscripciones abiertas",
      closedAtPrefix: "Cerrado:",
      reopenPlaceholder: "Motivo para reabrir",
      reopenButton: "Reabrir inscripciones",
      closePlaceholder: "Motivo del cierre",
      tabManual: "Manual",
      tabBulk: "Masiva",
      tabEntries: "Inscritos",
      tabSummary: "Resumen",
      manualClosedMsg:
        "Inscripciones cerradas. Reabre el torneo para inscribir jugadores manualmente.",
      bulkFeedbackSelected: "Seleccionados:",
      bulkFeedbackAdded: "Agregados:",
      bulkFeedbackSkipped: "Ya inscritos:",
      noTournamentSelected: "No hay torneo seleccionado.",
      loadErrorTitle: "No se pudo cargar inscritos",
      loadErrorBody:
        "Ocurrió un error al leer inscripciones, tarjetas o puntajes. Puedes cambiar de torneo o reintentar.",
      loadErrorTechnical: "Detalle técnico",
    },
  },
  teeSheet: {
    title: "Salidas/Grupos",
    loadErrorTitle: "No se pudo cargar salidas/grupos",
    loadErrorBody:
      "Puede deberse a permisos en Supabase (RLS), a una columna faltante en producción (migraciones pendientes) o a otro error de PostgREST. Revisa el detalle técnico abajo.",
    loadErrorTechnical: "Detalle del servidor",
    registrationOpenGate:
      "Inscripciones abiertas. Cierra inscripciones antes de generar salidas y grupos (R1, R2…).",
    priorRoundGate:
      "No se pueden generar salidas de la ronda {round} hasta cerrar la ronda {prior} en: {categories}.",
    priorRoundGateAction:
      "Cierra la ronda anterior para toda la categoría en la vista oficial o captura de tarjetas.",
    standingsOrderHint:
      "Orden según clasificación. El corte real solo aplica si en Reglas de corte hay una regla con destino R{round} (columna «A ronda»). Sin esa regla salen todos los inscritos.",
    pairingScoreHcp: "HCP",
    pairingScoreR1: "R1",
    pairingScoreR1R2: "R1+R2",
    cutEnforcesForRound:
      "R{round}: hay corte real al generar salidas (regla con destino R{round}). Solo pasan quienes figuren por encima de la línea en clasificación.",
    cutDoesNotEnforceForRound:
      "R{round}: sin corte real en salidas (no hay regla con destino R{round}). Salen todos los inscritos activos del bloque.",
    shotgunNoDoubleTees:
      "Con {groups} grupos en este bloque solo hay salidas A por hoyo. Las salidas B (1B, 2B…) empiezan en el grupo 19.",
  },
  rounds: {
    title: "Rondas",
    subtitle:
      "Configura el calendario del torneo por día, ronda, categoría, turno y horario.",
    newTournament: "+ Nuevo torneo",
    cutRulesLink: "Reglas corte",
    formSaving: "Guardando...",
    formDeleting: "Borrando...",
    btnDelete: "Borrar",
    confirmDeleteRound:
      "¿Borrar esta ronda? Si tiene grupos o scores relacionados, la base puede impedir el borrado.",
  },
  teeSets: {
    title: "Sets de salida",
    teeRulesLink: "Reglas de salidas",
  },
  courses: {
    title: "Campos de golf",
    shortNameLabel: "Nombre Corto",
    shortNamePlaceholderCourse: "Abrev. ej. CCQ",
    shortNamePlaceholderNewClub: "Abrev. del club",
    teeSetsSection: "Salidas base del campo",
    teeSetsHint:
      "Captura slope y course rating oficiales (WHS). Caballeros y damas pueden diferir en la misma salida (ej. Blancas).",
    coursesListTitle: "Campos dados de alta",
    coursesListHint:
      "Abre cualquier campo para revisar o capturar slope, rating y salidas WHS.",
    thClub: "Club",
    thCourse: "Campo",
    thTees: "Salidas",
    thWhsStatus: "WHS",
    thAction: "",
    whsComplete: "Completo",
    whsPartial: "Parcial",
    whsEmpty: "Sin datos",
    btnOpen: "Abrir",
    btnSelected: "Seleccionado",
    coursesEmpty: "Aún no hay campos dados de alta.",
    thOrder: "ORDEN",
    thCode: "CODE",
    thName: "NOMBRE",
    thColor: "COLOR",
    thGender: "GÉN.",
    thRatingMen: "RATING M",
    thSlopeMen: "SLOPE M",
    thRatingWomen: "RATING F",
    thSlopeWomen: "SLOPE F",
    genderEmpty: "—",
    genderM: "M",
    genderF: "F",
    genderX: "Mixto",
    saveTeeSets: "Guardar salidas base",
    whsColumnsMissing:
      "Las columnas WHS (slope/rating) aún no están en la base de datos. Aplica la migración 20260525210000_course_tee_sets_whs.sql en Supabase.",
  },
  clubs: {
    title: "Clubes",
    subtitle: "Catálogo maestro de clubes, logos oficiales y logos automáticos",
    searchPlaceholder: "Buscar club...",
    placeholderShortName: "Nombre Corto",
    thShortName: "Nombre Corto",
    sectionCreateTitle: "Alta de club",
    placeholderClubName: "Nombre del club",
    placeholderLogoUrlOptional: "URL logo oficial opcional",
    labelActive: "Activo",
    labelInactive: "Inactivo",
    saving: "Guardando...",
    btnCreateClub: "Crear club",
    sectionMergeTitle: "Fusionar club duplicado",
    sectionMergeDesc:
      "Mueve los campos y jugadores del club duplicado al club destino y deja el duplicado inactivo. No borra el registro origen.",
    mergeSelectSource: "Club duplicado / origen",
    mergeSelectTarget: "Club bueno / destino",
    merging: "Fusionando...",
    btnMerge: "Fusionar",
    clubOptionWithCourses: "{name} · campos: {count}",
    thLogo: "Logo",
    thClub: "Club",
    thCourses: "Campos",
    thOfficialLogo: "Logo oficial",
    thStatus: "Estatus",
    thActions: "Acciones",
    emptyResults: "Sin resultados.",
    labelColor: "Color:",
    placeholderPasteOfficialLogoUrl: "Pega URL de logo oficial",
    btnSaveLogo: "Guardar logo",
    cancel: "Cancelar",
    badgeOfficial: "Oficial",
    badgeAutomatic: "Automático",
    hintUseShortNameInitials: "Usa las iniciales del nombre corto.",
    previewOfficialLogo: "Logo oficial",
    previewAutomaticLogo: "Logo automático",
    btnSave: "Guardar",
    btnEdit: "Editar",
    btnLogo: "Logo",
    btnRegenerate: "Regenerar",
    deactivate: "Desactivar",
    activate: "Activar",
    dash: "—",
    errCreateNotConfirmed: "No se confirmó el alta del club.",
    errUpdateNotConfirmed: "No se confirmó la actualización del club.",
    errShortNameMismatch:
      "La base no devolvió el nombre corto esperado. No se guardó correctamente.",
    errLogoUpdateNotConfirmed: "No se confirmó la actualización del logo.",
    errToggleNotConfirmed: "No se confirmó el cambio de estatus.",
    errClubNotFoundEdit: "No se encontró el club a editar.",
    errClubNotFoundLogo: "No se encontró el club para editar logo.",
    errMergeSelectBoth: "Selecciona club duplicado y club destino.",
    errMergeSame: "El club duplicado y el destino no pueden ser el mismo.",
    errGenericCreateClub: "Error creando club",
    errGenericUpdateClub: "Error actualizando club",
    errGenericUpdateLogo: "Error actualizando logo",
    errGenericRegenerateLogo: "Error regenerando logo",
    errGenericToggle: "Error cambiando estatus",
    errGenericMerge: "Error fusionando club",
    infoClubCreated: "Club creado correctamente.",
    infoClubUpdated: "Club actualizado correctamente.",
    infoLogoUpdated: "Logo actualizado correctamente.",
    infoLogoRegenerated: "Logo automático regenerado.",
    infoClubDeactivated: "Club desactivado correctamente.",
    infoClubActivated: "Club activado correctamente.",
    infoMergeDone:
      "Fusión aplicada. Se movieron los campos y jugadores al club destino y el club duplicado quedó inactivo.",
  },
  scoreEntry: {
    title: "Captura de tarjetas",
    subtitle:
      "Busca al jugador por número de inscripción o nombre. El sistema asigna categoría, día, turno y ronda según inscripción, salidas y rondas.",
    subtitleCapture:
      "Captura rápida de la ronda abierta. Busca por número de inscripción o nombre.",
    subtitleModify:
      "Corrección de rondas cerradas o anteriores. Elige la ronda, ABRIR, edita y cierra de nuevo.",
    tabCapture: "Captura",
    tabModify: "Modificar scores",
    tabNavAria: "Modo de captura de tarjetas",
    searchHintCapture:
      "Solo la ronda abierta del jugador. Para corregir una ronda cerrada usa la pestaña Modificar scores.",
    searchHintModify:
      "Elige la ronda arriba. En rondas cerradas: ABRIR → corregir → Cerrar de nuevo.",
    captureClosedBanner:
      "La ronda en pantalla ya está cerrada. La captura diaria solo admite la ronda abierta.",
    captureClosedLink: "Ir a Modificar scores →",
    captureNextRoundLink: "Capturar R{round} (ronda siguiente)",
    labelSession: "Día y turno",
    roundsNeedDate:
      "Hay rondas sin fecha. Asigna fecha a cada ronda en la configuración del torneo para usar captura.",
    searchAmbiguousTitle:
      "Varios jugadores coinciden con la búsqueda. Elige la inscripción correcta:",
    searchNotFound: "No encontré jugador inscrito con «{q}».",
    priorRoundGate:
      "No se puede capturar la ronda {round}: la ronda {prior} debe estar cerrada para todos los jugadores activos de la categoría {category}.",
    registrationOpenGate:
      "Inscripciones abiertas. Cierra inscripciones antes de capturar tarjetas en torneo.",
    labelTournament: "Torneo",
    selectTournament: "Selecciona un torneo",
    selectTournamentHint:
      "Elige el torneo antes de buscar al jugador. Así la captura y las rondas coinciden con el evento correcto.",
    roundSwitcherKicker: "Ronda a capturar o corregir",
    roundCurrent: "En pantalla",
    roundClosed: "Cerrada · pulsa ABRIR abajo",
    roundOpen: "Con captura",
    roundEmpty: "Sin captura",
    misalignedCapture:
      "Hay scores guardados en otra fila de ronda de este torneo. Elige la ronda correcta arriba o usa «Reparar capturas» (directores).",
    playerFoundHint:
      "Para corregir R1 con R2 ya iniciada: elige «R1» arriba, luego «ABRIR», corrige y guarda.",
  },
  scorecards: {
    title: "Tarjeta de puntuación (prueba con Supabase)",
    subtitle:
      "Firma digital completa: marcador + jugador + testigo.",
  },
  caddies: {
    title: "Caddies",
    subtitle: "Asignación operativa de caddies por torneo y ronda",
    catalog: "Alta / Catálogo",
    tournaments: "Torneos",
    entries: "Inscripciones",
    scoreEntry: "Captura de tarjetas",
    filters: "FILTROS",
    filtersHint: "Filtra asignaciones por torneo y ronda",
    assignHeading: "ASIGNAR CADDIE POR RONDA",
    assignments: "ASIGNACIONES",
    telegramLink: {
      title: "Telegram del caddie",
      migrationWarning:
        "Falta migración en Supabase: columnas telegram_user_id y telegram_chat_id en caddies. Ejecuta la migración 20260527160000_caddies_telegram_link.sql.",
      statusLinked: "Telegram vinculado (user ID guardado).",
      statusNotLinked: "Aún no hay Telegram vinculado.",
      step1Title: "1. Abrir el bot en Telegram",
      step1BodyConfigured:
        "El caddie debe abrir el bot, pulsar «Iniciar» y escribir HOLA.",
      step1BodyNotConfigured:
        "Configura NEXT_PUBLIC_TELEGRAM_BOT_USERNAME en el servidor.",
      openBotLink: "Abrir bot en Telegram",
      botUsernameLabel: "Bot del torneo",
      copyBotUsername: "Copiar @usuario",
      copied: "Copiado",
      step2Title: "2. Obtener el ID si el bot no lo reconoce",
      step2Body:
        "Pide que escriba HOLA o ID al bot. El bot responderá con un número entre líneas ━━━━. Abajo aparecen los últimos que escribieron sin vincular.",
      pendingLinksTitle: "Últimos mensajes al bot (sin vincular)",
      pendingLinksHint:
        "Si el caddie ya escribió HOLA, busca su nombre y pulsa «Usar este ID».",
      pendingLinksEmpty:
        "Nadie sin vincular ha escrito al bot recientemente.",
      pendingLinksUseId: "Usar este ID",
      step3Title: "3. Guardar ID en la ficha",
      step3Body:
        "Guarda el ID para que el bot lo identifique y pueda recibir links de captura por grupo.",
      formTitle: "Registrar ID en la ficha del caddie",
      fieldUserId: "Telegram user ID (solo números)",
      fieldChatId: "Chat ID (opcional)",
      fieldChatHint:
        "Suele coincidir con el user id en chats privados; déjalo vacío si no lo necesitas.",
      btnSave: "Guardar vínculo",
      btnClear: "Quitar vínculo Telegram",
      verifyTitle: "Verificar enlace",
      verifyBody:
        "Envía un mensaje de prueba al Telegram del caddie.",
      btnVerify: "Enviar verificación por Telegram",
      savedBanner: "Vínculo guardado correctamente.",
      verifiedBanner: "Mensaje de verificación enviado.",
      editTelegram: "Vincular Telegram",
    },
  },
  tournamentHoles: {
    title: "Hoyos del torneo",
  },
  categories: {
    title: "Categorías",
  },
  cutRules: {
    title: "Reglas de Corte",
    noTournaments:
      "No hay torneos creados todavía. Primero crea un torneo para configurar reglas de corte.",
    linkTeeRules: "Reglas de Salidas",
    linkCompetition: "Competencia",
    savedOk: "Reglas guardadas correctamente.",
    editor: {
      prerequisiteBanner:
        "Debes tener rondas y categorías creadas en este torneo para configurar cortes.",
      newRule: "Nueva regla",
      helperLine:
        "Cortes por categoría: cupo exacto (% redondeado a la baja sobre inscritos). En el límite del corte aplica el perfil de desempate (nunca pasan todos los empatados).",
      thOrder: "Orden",
      thCategories: "Categorías",
      thFrom: "De",
      thTo: "A",
      thBasis: "Base",
      thMode: "Modo",
      thType: "Tipo",
      thCut: "Corte",
      thTies: "Empates",
      thGrossProtection: "Protección Gross",
      thTieBreak: "Desempate",
      thActive: "Activo",
      thNotes: "Notas",
      thActions: "Acciones",
      noRulesYet: "No hay reglas todavía.",
      selectedPrefix: "Seleccionadas:",
      pickOneCategory: "Selecciona al menos una categoría",
      passAll: "Pasan todos",
      topGross: "Top Gross",
      noExemption: "No",
      noTieBreak: "Sin desempate",
      titleMoveUp: "Subir",
      titleMoveDown: "Bajar",
      delete: "Borrar",
      saveRules: "Guardar reglas",
      observationsPlaceholder: "Observaciones",
      roundOptionPrefix: "R",
      addRowNeed:
        "Primero debes crear rondas y categorías para definir reglas.",
      valNeedRound: "Primero debes crear al menos 1 ronda.",
      valNeedCategories: "Primero debes crear categorías.",
      valFromRoundInvalid: "Ronda origen inválida en fila {row}.",
      valToRoundInvalid: "Ronda destino inválida en fila {row}.",
      valToRoundOrder:
        "La ronda destino debe ser mayor o igual que la origen en fila {row}.",
      valPickCategory: "Selecciona al menos una categoría en fila {row}.",
      valCutValueInvalid: "Valor de corte inválido en fila {row}.",
      valTopN: "Top N debe ser mayor o igual a 1 en fila {row}.",
      valTopPercent: "Top % debe estar entre 0 y 100 en fila {row}.",
      valGrossExempt:
        "Top Gross protegido debe ser mayor o igual a 1 en fila {row}.",
      valNeedTieBreak:
        "Selecciona perfil de desempate en fila {row} (obligatorio para cupo exacto).",
      valCutScopeGroup:
        "El grupo de categorías debe tener al menos 2 letras o ser un código exacto (ej. DE), no una sola letra como «D» (fila {row}).",
      rankingBasisLabels: {
        gross_total: "Gross total",
        net_total: "Neto total",
        points_total: "Puntos total",
        gross_round: "Gross ronda",
        net_round: "Neto ronda",
        points_round: "Puntos ronda",
      },
      rankingModeLabels: {
        tournament_to_date: "Acumulado torneo",
        specified_rounds: "Rango De/A",
        last_round_only: "Solo última ronda",
      },
      advancementLabels: {
        top_n: "Top N",
        top_percent: "Top %",
        all: "Sin corte / pasan todos",
      },
    },
  },
  categoryTemplates: {
    title: "Plantillas de categorías",
    headerTemplate: "PLANTILLA",
    headerNew: "NUEVA PLANTILLA",
  },
  categoryTeeRules: {
    title: "Reglas de Salidas por Categoría",
    linkTeeSets: "Sets de salida",
  },
  competitionRules: {
    title: "Reglas de Competencia",
    noTournaments:
      "No hay torneos creados todavía. Primero crea un torneo para configurar reglas de competencia.",
    linkCuts: "Cortes",
    linkPrizes: "Premios",
    linkCompetition: "Competencia",
    savedOk: "Reglas guardadas correctamente.",
    editor: {
      unnamedCategory: "Sin nombre",
      basisGross: "Gross",
      basisNet: "Neto",
      basisBoth: "Ambos",
      basisStableford: "Stableford",
      scoringStrokePlay: "Stroke Play",
      scoringStableford: "Stableford",
      netPlacesAll: "Todos",
      valMissingTournamentId: "Falta tournament_id.",
      valNoCategories: "Este torneo no tiene categorías para configurar.",
      valMissingCategoryId: "Falta category_id en {label}.",
      valHandicapNotNumber: "% handicap inválido en {label}.",
      valHandicapRange: "% handicap debe estar entre 0 y 150 en {label}.",
      valGrossPlacesInvalid: "Lugares Gross inválidos en {label}.",
      valGrossPlacesNegative: "Lugares Gross no puede ser negativo en {label}.",
      valNetPlacesInvalid: "Lugares Neto inválidos en {label}.",
      valNetPlacesNegative: "Lugares Neto no puede ser negativo en {label}.",
      valSfLbMismatch:
        "Si {label} es Stableford, el leaderboard debe ser Stableford.",
      valSfPrizeMismatch:
        "Si {label} es Stableford, los premios deben ser Stableford.",
      valSpLbSf:
        "Si {label} es Stroke Play, el leaderboard no puede ser Stableford.",
      valSpPrizeSf:
        "Si {label} es Stroke Play, los premios no pueden ser Stableford.",
      rowFallback: "fila {n}",
      introBlurb:
        "Configura la modalidad por categoría antes de calcular cortes, leaderboard y premios.",
      badgeCategories: "Categorías:",
      badgeActive: "Activas:",
      badgeStableford: "Stableford:",
      badgeHcp80: "HCP 80%:",
      filterPlaceholder: "Buscar categoría, modalidad o base",
      netEmptyHint:
        "Neto vacío = premiar a todos los demás después de Gross.",
      thOrder: "Orden",
      thCategory: "Categoría",
      thFormat: "Modalidad",
      thPrizes: "Premios",
      thHcpPct: "% HCP",
      thGross: "Gross",
      thNet: "Neto",
      thActive: "Activa",
      thNotes: "Notas",
      thSummary: "Resumen",
      noFilterMatch: "No hay categorías que coincidan con el filtro.",
      observationsPlaceholder: "Observaciones",
      summaryLbPrefix: "LB",
      summaryPrizesHandicap: "Premios {prizeBasis} · HCP {pct}%",
      summaryGrossNet: "Gross {gross} · Neto {net}",
      saveButton: "Guardar reglas de competencia",
      saveFooterNote:
        "Se guardan todas las categorías del torneo para que el leaderboard y premios lean una sola fuente.",
    },
  },
  convocatoria: {
    title: "Convocatoria",
    intro:
      "Arma la convocatoria en el sistema (plantilla 68º Torneo Anual), edita todas las variables, ciérrala y genera los parámetros del torneo. Los módulos manuales siguen disponibles después.",
    noTournaments: "Crea un torneo primero.",
    newTournament: "Nuevo torneo",
    linkPrizes: "Premios",
    startFromTemplate: "Empezar desde plantilla machote",
    templateHint:
      "Incluye categorías Camp–C, D-E, Seniors, Damas, cortes al 50 %, competencia gross/neto/Stableford y premios de podio del 68º Anual CCQ. Todo es editable.",
    loadTemplateButton: "Recargar plantilla 68º Torneo Anual (CCQ)",
    autoProvisionNote:
      "Plantilla 68º Torneo Anual cargada automáticamente. Revisa pestañas y guarda cuando quieras.",
    autoSyncNote:
      "Cortes y premios actualizados a la convocatoria 68º. Revisa y guarda el borrador.",
    rulesUpgradedNote:
      "Reglas de corte en BD actualizadas: acumulado 36 hoyos (R1+R2) y perfiles de desempate CCQ.",
    reloadTemplateHint:
      "Reemplaza el borrador en edición por la plantilla machote (pierdes cambios no guardados en BD).",
    importOptionalTitle: "Importar Word (opcional)",
    importOptionalHint:
      "Solo si tienes un .docx distinto; reemplaza el borrador en edición.",
    importOptionalButton: "Importar .docx",
    statusEditing: "En edición",
    statusClosed: "Cerrada — lista para generar parámetros",
    statusApplied: "Parámetros ya generados en el torneo",
    tabMeta: "General",
    tabReference: "Texto convocatoria",
    tabCategories: "Categorías",
    tabCompetition: "Competencia",
    tabCuts: "Cortes",
    tabPrizes: "Premios",
    saveDraft: "Guardar borrador",
    closeConvocatoria: "Cerrar convocatoria",
    reopenConvocatoria: "Reabrir para editar",
    generateParams: "Generar parámetros del torneo",
    generateBlocked:
      "Hay inscripciones: no se puede regenerar. Ajusta manualmente en admin.",
    confirmClose:
      "Al cerrar ya no podrás editar hasta reabrir. Luego podrás generar parámetros.",
    confirmGenerate:
      "Escribe categorías, competencia, cortes, premios y rondas en el torneo.",
    closedOk:
      "Convocatoria cerrada. Revisa el borrador y pulsa «Generar parámetros del torneo».",
    appliedOk:
      "Parámetros generados. Revisa Hoyos del torneo, Sets de salida y desempates.",
    appliedOkMatchPlay:
      "Parámetros match play generados. Revisa categorías, rondas del cuadro y luego inscribe parejas.",
    introMatchPlay:
      "Arma la convocatoria match play por parejas: formato, cuadro, categorías y desempates. Cierra y genera parámetros sin afectar torneos stroke/stableford.",
    autoProvisionNoteMatchPlay:
      "Plantilla match play cargada automáticamente. Revisa reglas y categorías.",
    linkMatchPlay: "Match play",
    tabMatchPlayRules: "Reglas match play",
    matchPlayBadge: "Match Play",
    confirmGenerateMatchPlay:
      "Crea categorías, premios, rondas del cuadro y reglas match play en el torneo.",
    readOnlyHint: "Convocatoria cerrada o aplicada — solo lectura",
    metaTitle: "Título",
    metaHoles: "Hoyos totales",
    metaCutHoles: "Corte tras (hoyos)",
    metaCutPct: "Corte máx. %",
    metaRounds: "Rondas",
    metaPracticeDay: "Día de práctica",
    metaHandicapDate: "Fecha índice de handicap",
    refSystem: "Sistema de juego",
    refGentlemen: "Caballeros",
    refLadies: "Damas",
    refSeniorsAges: "Edades Seniors",
    refCutPolicy: "Política de corte",
    refCutTiebreakGross: "Desempate corte (gross)",
    refCutTiebreakStableford: "Desempate corte (Stableford)",
    refCutTiebreakSeniors: "Desempate corte (Seniors)",
    refTrophyTiebreak: "Desempate trofeos",
    refTrophies: "Trofeos y premios especiales",
    refOutOfScope: "Fuera de parámetros automáticos",
    reviewWarnings: "Avisos",
    colCode: "Cód.",
    colName: "Nombre",
    colHcp: "H.I.",
    colCut: "Corte",
    colGroup: "Grupo",
    colGender: "Género",
    addCategory: "Añadir categoría",
    migrationMissing:
      "Falta la tabla tournament_convocatoria en Supabase. Ejecuta las migraciones 20260518120000 y 20260518130000 antes de usar este módulo.",
  },
  prizeRules: {
    title: "Reglas de Premios",
    linkCut: "Reglas de Corte",
    linkLeaderboard: "Clasificación",
    noTournaments:
      "No hay torneos creados todavía. Primero crea un torneo para configurar premios.",
  },
  users: {
    title: "USUARIOS",
  },
  tournaments: {
    title: "TORNEOS",
    colEntries: "Inscripciones",
    subtitle: "Control maestro de operación",
    linkPublicHome: "Home pública",
    linkPublicTournaments: "Torneos públicos",
    newTournament: "Nuevo torneo",
    searchHeading: "BUSCADOR / ORDENADOR",
    searchHint:
      "Filtra por club anfitrión (solo clubes con al menos un torneo) y por rango de fechas de inicio.",
    labelClub: "Club anfitrión",
    clubSelectAll: "Todos los clubes",
    labelDateFrom: "Fecha desde",
    labelDateTo: "Fecha hasta",
    filter: "Filtrar",
    clear: "Limpiar",
    allByDate: "Todos por fecha",
    filterShowing: "Mostrando",
    tournamentOne: "torneo",
    tournamentMany: "torneos",
    filteredOne: "filtrado",
    filteredMany: "filtrados",
    byClub: "por club",
    wordFrom: "desde",
    wordTo: "hasta",
    orderPrefix: "Orden:",
    orderNewestFirst: "más reciente a más viejo",
    filterAllIntro: "Mostrando",
    filterAllTournaments: "todos los torneos",
    filterAllOrdered: "ordenados de",
    thDate: "Fecha",
    thTournament: "Torneo",
    thStatus: "Estatus",
    thPublic: "Público",
    thClub: "Club",
    thCourse: "Campo",
    thHoles: "Hoyos",
    thCategories: "Categorías",
    thTeeSets: "Salidas",
    thRules: "Reglas salidas",
    thRounds: "Rondas",
    thStaff: "Staff",
    thActions: "Acciones",
    thConvocatoria: "Convocatoria",
    thCompetition: "Competencia",
    thCuts: "Cortes",
    thPrizes: "Premios",
    noPoster: "Sin poster",
    clubLine: "Club:",
    courseLine: "Campo:",
    btnPublic: "Pública",
    edit: "Editar",
    scores: "Scores",
    hide: "Ocultar del sitio público",
    show: "Publicar en el sitio público",
    showArchivedHint: "Reactiva el torneo para poder publicarlo.",
    reactivate: "Reactivar",
    archive: "Archivar",
    updating: "Actualizando...",
    emptyWithFilters: "No hay torneos registrados con esos filtros.",
    visibility: {
      archived: "Archivado",
      public: "Público",
      hidden: "Oculto",
    },
  },
  courseHoles: {
    title: "Tarjeta base del campo",
  },
  tournamentSetup: {
    courseWithoutName: "Campo sin nombre",
    unnamed: "Sin nombre",
    noCategoriesLoaded: "Sin categorías cargadas",
    noRoundsLoaded: "Sin rondas cargadas",
    pageTitle: "Configuración de torneo",
    pageSubtitle:
      "Torneos existentes · generar campos, aplicar plantilla y preparar operación.",
    labelTournament: "Torneo",
    selectTournament: "Selecciona torneo",
    change: "Cambiar",
    newTournament: "Nuevo torneo",
    summaryTitle: "Resumen del torneo",
    linkCategories: "Categorías",
    linkStarters: "Salidas",
    linkRounds: "Rondas",
    linkScoreEntry: "Captura scores",
    linkHandicapCommittee: "Comité Handicap",
    infoTournament: "Torneo",
    infoClub: "Club",
    infoCourse: "Campo",
    infoAbbr: "Abrev.",
    infoStart: "Inicio",
    infoCategories: "Categorías",
    quickInitTitle: "Inicialización rápida",
    quickInitSubtitle:
      "Genera campo del torneo, categorías desde plantilla y rondas base en una sola acción.",
    currentCourse: "Campo actual",
    currentCategories: "Categorías actuales",
    currentRounds: "Rondas actuales",
    labelClub: "Club",
    all: "Todos",
    viewCourses: "Ver campos",
    baseCourse: "Campo base",
    changeBaseCourse: "Cambiar campo base",
    selectCourse: "Selecciona campo",
    modifyCategories: "Modificar categorías",
    leaveCurrentCategories: "Dejar categorías actuales",
    selectTemplate: "Selecciona plantilla",
    replaceCurrentCategories: "Cambiar categorías actuales",
    roundsShort: "Rondas",
    initTournament: "Inicializar torneo",
    initWarning:
      "Reemplaza tournament_holes y rounds. Si eliges plantilla, también reemplaza categories. Si el torneo no tiene fecha de inicio, las rondas se crearán usando la fecha de hoy.",
    sectionCourse: "Campo",
    sectionCourseDesc:
      "Selecciona club y campo base para generar la tarjeta del torneo en tournament_holes.",
    generateCourseFields: "Generar campos del torneo",
    replaceHolesOnly:
      "Esta acción reemplaza los registros actuales de tournament_holes.",
    thHole: "HOYO",
    thPar: "PAR",
    thHandicap: "VENTAJA",
    noCourseGenerated: "Este torneo todavía no tiene campos generados.",
    sectionCategories: "Categorías",
    sectionCategoriesDesc: "Aplica una plantilla existente al torneo actual.",
    applyTemplate: "Aplicar plantilla",
    currentCategoriesPrefix: "Categorías actuales:",
    deleteCategoriesWarning:
      "Esta acción elimina primero las categorías actuales del torneo y luego carga las de la plantilla seleccionada.",
    noActiveTemplates: "No hay plantillas activas disponibles.",
    fineTuneHint: "Después puedes afinar detalles en el módulo de categorías.",
    sectionStarters: "Salidas",
    sectionStartersDesc:
      "Bloque listo para seleccionar salidas y reglas por torneo.",
    openStarters: "Abrir salidas",
    teeRules: "Reglas de salidas",
    sectionLeagueConvocatoria: "Liga y convocatoria",
    sectionLeagueConvocatoriaDesc:
      "Arma la convocatoria (plantilla, cortes, premios y reglas) o revisa la liga y premios por categoría.",
    linkConvocatoria: "Convocatoria",
    linkCompetitionRules: "Liga",
    linkPrizeRules: "Premios",
    sectionRoundsType: "Tipo de torneo y rondas",
    sectionRoundsTypeDesc:
      "Desde aquí puedes continuar con parámetros operativos, rondas y reglas complementarias.",
    openRounds: "Abrir rondas",
    linkCutRules: "Reglas de corte",
    noTournamentsBefore: "No hay torneos disponibles. Primero crea uno en",
    noTournamentsPath: "/tournaments/new",
  },
  editors: {
    leaderboardColumn: "Clasificación",
  },
  publicTournament: {
    home: "← Inicio",
    seeTournaments: "Ver torneos",
    adminList: "Ir a lista de torneos",
    live: "Resultados en vivo",
    leaderboard: "Clasificación",
    teeSheet: "Salidas/Grupos",
    favorites: "Favoritos",
    cercanosTab: "Más cerca",
    convocatoria: "Convocatoria",
    bracket: "Cuadro",
    bracketLiveTab: "Bracket en vivo",
    matchesLiveTab: "Matches en vivo",
    pageTitleBracket: "Cuadro match play",
    pageDescBracket:
      "Partidos en vivo con puntos por hoyo (bola baja y bola alta).",
    bracketEmpty:
      "El cuadro aún no está publicado. El comité debe publicarlo desde administración.",
    bracketFormat: "Formato",
    bracketAllowance: "Hándicap",
    bracketVs: "vs",
    bracketHoleDetail: "Detalle por hoyo",
    bracketLowBall: "Baja",
    bracketHighBall: "Alta",
    bracketPoints: "pts",
    bracketLive: "En juego",
    bracketCompleted: "Finalizado",
    bracketBye: "BYE",
    eSignature: "✍️ Firma electrónica",
    posterAlt: "Poster torneo",
    allCategories: "Todas las categorías",
    publicBadge: "Torneo público",
    playerOne: "jugador",
    playersMany: "jugadores",
    categoryChip: "Categoría:",
    roundChip: "Ronda",
    pageTitleLive: "Resultados en vivo",
    pageTitleOfficial: "Resultados oficiales",
    pageTitleFavorites: "Mis favoritos",
    pageTitleTeeSheet: "Salidas/Grupos",
    pageTitleConvocatoria: "Convocatoria",
    pageDescConvocatoria:
      "Reglas y condiciones del torneo publicadas por el comité. Documento de solo lectura.",
    convocatoriaEmpty:
      "La convocatoria aún no está publicada para este torneo.",
    convocatoriaReadOnlyNote:
      "Documento informativo. No se puede editar desde esta página.",
    convocatoriaGeneralHeading: "Datos generales",
    convocatoriaCategoriesHeading: "Categorías",
    pageDescOfficial:
      "Resultados verificados por la administración del torneo.",
    pageDescFavorites:
      "Seguimiento rápido de los jugadores marcados como favoritos.",
    favoritesAllCategoriesHint:
      "Todos los jugadores que marques con ★, de cualquier categoría. POS, thru y totales según su ronda actual.",
    pageDescTeeSheet:
      "Lista pública de salidas por día, visible solo cuando el comité confirma el orden definitivo.",
    pageDescLive:
      "Resultados en tiempo real del torneo con avances de captura y posiciones por categoría.",
    statRounds: "Rondas",
    statStarts: "Salidas",
    statPending: "Pendientes",
    cardLiveKicker: "Resultados en vivo",
    cardLiveTitle: "Seguimiento en vivo",
    cardLiveBody:
      "Captura visible públicamente sin entrar al sistema administrativo.",
    cardTeeKicker: "Salidas/Grupos",
    cardTeeTitle: "Grupos por día",
    cardTeeBody:
      "Las salidas públicas aparecen cuando el comité confirma el orden definitivo del día.",
    cardSignKicker: "Firma electrónica",
    cardSignTitle: "Acceso directo a firmas",
    cardSignBody:
      "Ingresa a la captura y validación de firma del jugador y marker.",
    cardSignCta: "Ir a firma electrónica",
    officialChipClosed: "cerradas",
    officialChipPending: "faltan",
    officialChipDone: "completo",
    officialCloseRoundLabel: "Ronda {n} · estado por categoría",
    officialShowPendingList: "Ver {n} pendientes",
    officialHidePendingList: "Ocultar lista",
    officialPendingHeading: "Pendientes de cerrar",
    officialCaptureCta: "Capturar",
    officialPendingManyHint: "Faltan {n} jugadores por cerrar",
    liveRoundToday: "Hoy",
    liveRoundHistory: "Historial",
    liveRoundFuture: "Próximas fechas",
    liveRoundUndated: "Sin fecha en calendario",
    viewingRoundKicker: "Viendo",
    noRoundSelected: "Elige categoría; la ronda por defecto es la de hoy para esa categoría.",
    historicRoundsToggle: "Histórico: otras fechas y rondas",
    detailHolesCol: "HOYOS",
    detailParRow: "Par",
    detailFirstNineTitle: "1ª",
    detailFirstNineSub: "",
    detailSecondNineTitle: "2ª",
    detailSecondNineSub: "",
    detailTotalTitle: "TOT",
    detailTotalSub: "Total 18 · suma 1ª+2ª",
    detailTotalsPlacement: "inline-after-nines" as
      | "inline-after-nines"
      | "trailing-after-18",
    detailGross: "GROSS",
    detailToPar: "TO PAR",
    detailPos: "POS",
    detailNoCapture: "No hay captura para la ronda seleccionada.",
    detailExpandedCategoryLabel: "Categoría",
    detailExpandedClubLabel: "Club",
    detailCloseButton: "Cerrar",
    detailCloseAria: "Cerrar detalle de puntuación",
    detailToggleOpenAria: "Ver detalle de puntuación",
    detailToggleCloseAria: "Ocultar detalle de puntuación",
    leaderboardEmptyView: "No hay jugadores para mostrar en esta vista.",
    playerSearchPlaceholder: "Buscar jugador, club o categoría…",
    playerSearchAria: "Filtrar la tabla por nombre, club o categoría",
    playerSearchHint:
      "Filtra la vista actual. Si no sabes la categoría, elige «Todas las categorías» arriba y busca aquí.",
    playerSearchNoMatches: "Ningún jugador coincide con tu búsqueda.",
    playerSearchCount: "{shown} de {total}",
    detailAuditStrokeIndex: "SI",
    detailAuditStrokesReceived: "HCP",
    detailAuditNetStrokes: "Net",
    detailAuditStablefordPoints: "Pts",
    categoryCompetitionTitle: "Competencia",
    categoryCompetitionModality: "Modalidad",
    categoryCompetitionLeaderboard: "Clasificación",
    categoryCompetitionPrizes: "Premios",
    categoryCompetitionGrossPlaces: "{n} plazas gross",
    categoryCompetitionNetPlaces: "{n} plazas neto",
    categoryCompetitionStablefordPlaces: "Premios Stableford",
    categoryCompetitionGrossNetHint:
      "Usa las pestañas Gross / Neto para cambiar la clasificación mostrada.",
    basisToggleGross: "Gross",
    basisToggleNet: "Neto",
    basisToggleAria: "Ver clasificación gross o neto",
    prizesPanelTitle: "Premios",
    prizesConfiguredPlaces: "Plazas en competencia",
    prizesGrossPlace: "{n} gross",
    prizesNetPlace: "{n} neto",
    prizesStablefordPlace: "{n} Stableford",
    prizesNoDetailedRules: "Sin premios detallados publicados para esta categoría.",
    prizesBasisGross: "Gross",
    prizesBasisNet: "Neto",
    prizesBasisStableford: "Stableford",
    loadErrorTitle: "No se pudo cargar el torneo",
    loadErrorBody:
      "Ocurrió un error al leer datos del torneo. Intenta de nuevo en unos segundos.",
    loadErrorTechnical: "Detalle técnico",
    leaderboardBuildErrorTitle: "Error al armar la clasificación",
    leaderboardBuildErrorBody:
      "Los datos del torneo se leyeron, pero falló el cálculo de posiciones. Revisa la configuración en administración.",
    rulesBlockedTitle: "Clasificación no disponible",
    rulesBlockedIntro:
      "Hay problemas en la configuración del torneo. Corrige cada punto en administración antes de publicar resultados.",
    rulesServiceRoleNotConfigured:
      "Falta SUPABASE_SERVICE_ROLE_KEY en el servidor (Vercel → Environment Variables). Sin ella no se pueden leer las reglas de competición.",
    rulesCompetitionLoadFailed:
      "No se pudieron cargar las reglas de competición desde la base de datos.",
    rulesCutLoadFailed:
      "No se pudieron cargar las reglas de corte (necesarias desde la ronda 2).",
    rulesCategoriesMissingRule:
      "Falta regla de competición activa para: {codes}.",
    rulesCompetitionInvalidConfig:
      "Reglas de competición inválidas: {details}.",
    rulesStrokeIndexIncomplete:
      "Hoyos del campo incompletos: solo {count} de 18 tienen índice de handicap (requerido para neto/Stableford).",
    rulesBlockedAdminHint:
      "Administración → Competencia, Reglas de corte y Hoyos del torneo.",
  },
  publicTeeSheet: {
    empty:
      "Aún no hay salidas publicadas. El comité debe confirmar/cerrar el orden definitivo del día en Salidas/Grupos.",
    allDays: "Todos los días",
    noGroupsFilter: "No hay grupos para mostrar con este filtro.",
    publishedStarts: "Salidas publicadas",
    groupOne: "grupo",
    groupMany: "grupos",
    startingTee: "Salida",
    playerOne: "jugador",
    playersMany: "jugadores",
    scoreHcp: "HCP",
    scoreR1: "R1",
    scoreR1R2: "R1+R2",
  },
  publicInstall: {
    rowLabel: "Acceso",
    button: "Icono / acceso",
    aria: "Ver cómo crear acceso directo a listgolf.club",
    title: "Acceso rápido a List.golf",
    intro:
      "Crea un icono para abrir el torneo con un toque. Los pasos cambian según si usas iPhone, Android o Mac.",
    installNow: "Instalar en este dispositivo",
    stepsLead: "Sigue estos pasos (con imagen):",
    otherDevices: "Otras plataformas",
    close: "Entendido",
    highlightShare: "Compartir",
    highlightMenu: "Menú ⋮",
    ios: {
      title: "iPhone o iPad",
      note: "Usa Safari. En iPhone no existe «Añadir a inicio» en Chrome.",
      steps: [
        {
          text: "Abre listgolf.club en Safari (no dentro de WhatsApp ni Instagram).",
          visual: "ios-share",
        },
        {
          text: "Abajo, toca el botón Compartir (cuadrado con flecha ↑). Solo abre un menú.",
          visual: "ios-share",
          highlight: "Compartir",
        },
        {
          text: "En el menú, baja y toca «Añadir a inicio» (no Mensajes ni Mail).",
          visual: "ios-add-home",
        },
        {
          text: "Arriba a la derecha toca «Añadir». Sin ese paso no aparece el icono.",
          visual: "ios-confirm",
        },
      ],
    },
    android: {
      title: "Android",
      note: "Chrome u otro navegador similar.",
      steps: [
        {
          text: "Arriba a la derecha, toca el menú de tres puntos ⋮.",
          visual: "android-menu",
          highlight: "Menú ⋮",
        },
        {
          text: "Elige «Añadir a pantalla de inicio» o «Instalar app» y confirma.",
          visual: "android-install",
        },
      ],
    },
    macSafari: {
      title: "Mac con Safari",
      note: "En Mac no hay «Añadir a inicio» como en el iPhone. En Safari se usa «Añadir al Dock» (icono en la barra inferior).",
      steps: [
        {
          text: "Abre listgolf.club en Safari (no en Chrome).",
          visual: "mac-file-dock",
        },
        {
          text: "En la barra de menú arriba: Archivo → «Añadir al Dock…» (en inglés: File → Add to Dock).",
          visual: "mac-file-dock",
        },
        {
          text: "Confirma en el cuadro. El icono queda en el Dock de tu Mac.",
          visual: "mac-dock-result",
        },
      ],
    },
    macChrome: {
      title: "Mac con Chrome",
      note: "Puedes instalar la app web o crear un acceso en el Dock.",
      steps: [
        {
          text: "Busca el icono ⊕ «Instalar» en la barra de direcciones (a la derecha de la URL).",
          visual: "chrome-install",
        },
        {
          text: "Si no aparece: menú ⋮ → «Instalar List.golf…» o «Crear acceso directo».",
          visual: "chrome-install",
        },
      ],
    },
    macOther: {
      title: "Mac (otro navegador)",
      note: "Prueba Safari (Archivo → Añadir al Dock) o Chrome (Instalar).",
      steps: [
        {
          text: "Recomendado: abre el sitio en Safari y usa Archivo → Añadir al Dock.",
          visual: "mac-file-dock",
        },
        {
          text: "Alternativa: guarda un marcador en la barra de favoritos para acceso rápido.",
          visual: "bookmark-bar",
        },
      ],
    },
    desktopChrome: {
      title: "Computadora (Chrome / Edge)",
      note: "Instalación como app o acceso directo.",
      steps: [
        {
          text: "En la barra de direcciones, clic en «Instalar» o el icono ⊕.",
          visual: "chrome-install",
        },
        {
          text: "O menú del navegador → Instalar / Crear acceso directo.",
          visual: "chrome-install",
        },
      ],
    },
    desktopOther: {
      title: "Computadora",
      note: "Usa el marcador del navegador o prueba Chrome para instalar la app.",
      steps: [
        {
          text: "Arrastra el icono del candado o la URL a la barra de favoritos.",
          visual: "bookmark-bar",
        },
      ],
    },
  },
  capturaTelegram: {
    title: "Modo Telegram",
    subtitle:
      "Links de captura móvil por grupo. Compártelos por Telegram a jugadores y caddies.",
    pickTournament: "Torneo",
    pickRound: "Ronda",
    noTournament: "Selecciona un torneo para ver los grupos.",
    noRound: "Este torneo no tiene rondas. Crea una ronda primero.",
    noGroups:
      "La ronda seleccionada aún no tiene grupos. Genera grupos en Tee Sheet.",
    table: {
      group: "Grupo",
      hole: "Hoyo",
      teeTime: "Hora",
      players: "Jugadores",
      caddies: "Caddies",
      link: "Link captura",
      actions: "Acciones",
      noCaddies: "Sin caddies",
    },
    btnCopy: "Copiar",
    btnCopied: "¡Copiado!",
    btnQr: "QR",
    btnSend: "Enviar Telegram",
    btnSending: "Enviando…",
    btnSendAll: "Enviar a TODO el torneo",
    btnSendAllConfirm: "¿Enviar el link a todos los grupos?",
    qrModalTitle: "QR del grupo",
    qrModalHint:
      "El caddie o jugador puede escanear este QR para abrir el link de captura.",
    qrModalClose: "Cerrar",
    sendResultSuccess: "Enviado a {sent} destinatario(s).",
    sendResultPartial: "Enviado a {sent} · {failed} sin Telegram vinculado.",
    sendResultEmpty: "Ningún destinatario tiene Telegram vinculado.",
    sendResultError: "Error enviando por Telegram.",
    linkedYes: "TG",
    linkedNo: "—",
    legendTitle: "Cómo funciona",
    legendBody:
      "Los jugadores se vinculan desde Inscritos → KIT. Los caddies se vinculan escribiendo «HOLA» al bot del torneo. Solo a los vinculados se les puede enviar el link automáticamente.",
    statTitle: "Estado del torneo",
    statPlayersLinked: "Jugadores vinculados",
    statCaddiesLinked: "Caddies vinculados",
    statGroups: "Grupos",
    msgTitle: "📋 Captura por grupo",
    msgIntro:
      "Aquí está el link de la tarjeta del grupo. Cualquiera del grupo (jugador o caddie) puede capturar y todos verán los cambios en tiempo real.",
  },
  handicapCommittee: {
    pageTitle: "Comité de Handicap",
    pageSubtitle: "Voto anónimo · Ajuste al handicap de torneo (−1 a −5 golpes)",
    notLoggedIn: "Inicia sesión para acceder al comité.",
    noTournament: "Selecciona un torneo para usar el comité de handicap.",
    pickTournament:
      "Elige un torneo para auditar handicaps o votar (voto anónimo entre miembros).",
    pickOpenVote: "Elige la votación de comité a la que entrar.",
    changeTournament: "Cambiar torneo",
    noOpenVote: "No hay votación de comité abierta en este momento.",
    noDailyRounds:
      "El comité de handicap solo aplica a torneos, no a las jugadas del día.",
    noTournaments:
      "No tienes torneos asignados. Pide a un administrador que te asigne el rol «Comité de Handicap» o «Director del Torneo» en algún torneo.",
    noAccess: "No tienes acceso al comité de handicap de este torneo.",
    bannerEnabled:
      "Comité activado. Asigna el rol «Comité de Handicap» a los miembros en Usuarios.",
    bannerClosed: "Votación cerrada.",
    bannerHiApplied: "Handicap de torneo actualizado con el ajuste del comité.",
    bannerHiAppliedBulk: "Handicap de torneo actualizado para {n} jugador(es).",
    bannerHiAppliedBulkPartial:
      "Handicap de torneo actualizado para {ok} jugador(es). {fail} falló(s).",
    bannerPresent: "Miembro marcado como presente.",
    bannerAbsent: "Miembro marcado como ausente.",
    bannerRoleAssigned: "Usuario agregado al comité.",
    bannerRoleRevoked: "Usuario removido del comité.",
    bannerTrimSaved: "Recorte de outliers actualizado.",
    bannerVotesReset:
      "Votación reiniciada: se archivó la sesión anterior y todos los miembros deben volver a votar desde cero.",
    adminBadge: "Solo admin",
    adminScopeDesc:
      "Como Director / Club Admin / Super Admin puedes parametrizar reglas, marcar presentes, cerrar votación, reiniciar e historial.",
    btnVote: "Votar",
    btnAdmin: "⚙ Administración",
    btnManageMembers: "Gestionar miembros",
    notActive:
      "El comité aún no está activo en este torneo. Un administrador debe activarlo primero.",
    emptyNoEntries:
      "No hay inscripciones activas en este torneo todavía. Agrega jugadores en {entriesLink} para que el comité los pueda calificar.",
    entriesLinkLabel: "Inscripciones",
    emptyNoFlagged:
      "Ningún jugador en la lista de votación todavía. En {selectionLink} marca a quien el comité deba revisar: al marcar uno, aparece al instante para todos los miembros.",
    emptyNoFlaggedVoter:
      "Ningún jugador en la lista de votación todavía. Cualquier miembro puede marcarlos en {selectionLink}; al marcar uno, aparece para todos. No hay lista por persona.",
    selectionLinkLabel: "Selección de jugadores",
    voter: {
      presenceTitle: "No estás marcado como presente.",
      presenceAdmin:
        "Marca tu asistencia en la sección «Miembros del comité» del panel de Administración para poder votar.",
      presenceMember:
        "Pide al director del torneo que te active en el panel del comité. Mientras tanto no podrás guardar votos.",
      notCommitteeTitle: "Puedes ver el comité, pero no votar.",
      notCommitteeBody:
        "Solo quienes tienen el rol handicap_committee emiten voto. Un director, marshal o club_admin presente no mueve el handicap.",
      progressLabel: "Tu progreso",
      progressClosed:
        "La votación está cerrada. Ya no puedes cambiar tus calificaciones.",
      progressVotePrivate:
        "Voto privado. Solo bajar el handicap de torneo (−1 a −5 golpes).",
      tabPending: "Pendientes",
      tabRated: "Calificados",
      tabResults: "Resultados",
      searchPlaceholder: "Buscar jugador, club, HP…",
      clearSearch: "Limpiar búsqueda",
      pendingMissing: "Faltan",
      pendingFiltered: " (filtrados)",
      pendingRated: "Calificados",
      ratedShowing: "Mostrando",
      ratedSuffix: "calificados",
      ratedRemaining: "Faltan",
      ratedOf: "de",
      emptyPendingSearch: "Sin pendientes que coincidan con la búsqueda.",
      emptyPendingDone: "¡Listo! No te faltan jugadores por calificar.",
      emptyRatedSearch: "No tienes calificaciones que coincidan.",
      emptyRatedNone: "Aún no has calificado a ningún jugador.",
      closedHint:
        "La votación está cerrada. Cambia a «Resultados» para ver los promedios o a «Calificados» para revisar tus votos.",
      resultsEmpty: "Aún no hay resultados agregados para mostrar.",
      resultsLiveTitle: "Resultados parciales en vivo",
      resultsFinalTitle: "Resultados finales",
      resultsLiveBadge: "🔴 En vivo",
      resultsFinalBadge: "✓ Final",
      resultsHelp:
        "Toca el nombre del jugador (subrayado azul) para ver la distribución voto por voto y las abstenciones. Anónimo.",
      thPlayer: "Jugador",
      thHi: "HP",
      thHiTitle: "Handicap de torneo (golpes a jugar)",
      thVotes: "Votos",
      thVotesTitle: "Votos emitidos (ajustes + abstenciones)",
      thAdj: "Ajustes",
      thAdjTitle: "Ajustes numéricos que sobreviven al recorte",
      thAbst: "Abst.",
      thAbstTitle: "Abstenciones (no son ajustes)",
      thAvg: "Prom",
      thAvgTitle: "Promedio recortado del ajuste (golpes de HP)",
      thHiSug: "HP sug",
      thHiSugTitle: "HP sugerido = handicap de torneo + promedio en golpes",
      thVetos: "Vetos",
      thVetosTitle: "Votos de «No permitir jugar este torneo»",
      legendHi: "Handicap de torneo actual",
      legendVotes: "emitidos en total (ajustes + abstenciones)",
      legendAdj: "numéricos que sobreviven al recorte",
      legendAbstOut: "no entran al promedio",
      legendAbstInAvg: "cuentan como 0 en el denominador",
      legendAvg: "promedio recortado (golpes de HP)",
      legendHiSug: "HP + Promedio",
      legendVetos: "votos de «No permitir jugar»",
      expandedTitle: "Votos uno por uno (anónimos)",
      expandedNoVotes: "Aún no hay votos para este jugador.",
      expandedHi: "HP actual",
      expandedAvg: "Prom.",
      expandedHiSug: "HP sug.",
      chipAbstainOut: "Abstención (no entra al promedio)",
      chipAbstainInAvg: "Abstención (cuenta como 0 en el promedio)",
      chipTrimmedLow: "Descartado (más severo)",
      chipTrimmedHigh: "Descartado (más suave)",
      chipLive: "Voto vivo",
      chipAbst: "0·abst",
    },
    card: {
      ghinTitle: "GHIN del jugador",
      ghinChip: "GHIN",
      reviewBadge: "Revisar",
      hiLabel: "HP",
      chLabel: "CH",
      chTitlePrefix: "Slope",
      chTitleFallback: "Course Handicap",
      phLabel: "PH",
      phTitlePrefix: "Playing Handicap (allowance",
      phTitleFallback: "Playing Handicap",
      savedAria: "Voto guardado",
      savedJustNow: "Guardado",
      pendBadge: "Pend",
      voteOpenReport: "📄 Ver reporte GHIN",
      youAbstained: "Te abstuviste",
      onThisPlayer: "en este jugador.",
      ratingSavedPrefix: "Calificación guardada:",
      pts: "golpes",
      dqMarkedLine: "Marcaste: no permitir jugar este torneo",
      lockedHint: "Bloqueado · votación cerrada",
      editBtn: "Editar calificación",
      abstainBtn: "Sin opinión (abstenerse)",
      lowerHiLabel: "Bajar handicap de torneo",
      sliderMin: "−5 (máx.)",
      sliderMax: "−1 (mín.)",
      dqTitle: "No permitir jugar este torneo",
      dqHint:
        "Solo en casos excepcionales (conducta, trampa, riesgo grave). Voto anónimo.",
      dqBadgeMarked: "Marcado",
      dqBadgeTap: "Tocar",
      saveBtn: "Guardar calificación",
      saveBtnUpdate: "Guardar cambios",
      cancelBtn: "Cancelar",
      errorSave: "Error al guardar",
      tooLateClosed: "No alcanzaste a votar y la votación ya está cerrada.",
      summaryHiCurrent: "HP actual",
      summaryAvg: "Promedio",
      summaryHiSug: "HP sugerido",
      summaryLiveLong: "ajustes",
      summaryNumShort: "núm.",
      summaryAbstSuffix: "abst.",
    },
    admin: {
      sectionTitle: "Administración del comité",
      expectedMembers: "Miembros esperados",
      activate: "Activar comité en este torneo",
      statusLabel: "Estado:",
      statusOpen: "Abierta",
      statusClosed: "Cerrada",
      membersWithRole: "Miembros con rol:",
      expectedMembersHint:
        "Indicador de quórum esperado. No entra al recorte ni al umbral de descalificación.",
      presentToday: "Presentes hoy:",
      canCloseHint:
        "Puedes cerrar la votación aunque falten miembros por calificar a todos los jugadores.",
      btnClose: "Cerrar votación",
      btnReopen: "Reabrir votación",
      membersTitle: "Miembros del comité del club",
      membersHelp:
        "Define una sola vez quiénes son los miembros permanentes del comité (alcance «Todo el club»). En cada torneo solo marca cuáles están presentes para habilitar su voto en esa sesión.",
      attendanceTitle: "Presencia en este torneo",
      attendanceHint:
        "Marca con «✅ Marcar presente» a los miembros que asistirán a la votación de ESTE torneo. Los demás quedan inactivos esta sesión, pero conservan su rol en el club.",
      addMember: "Agregar miembro al comité del club",
      chooseUser: "— elige un usuario —",
      scopeLabel: "Alcance",
      scopeTournament: "Solo este torneo",
      scopeClub: "Todo el club",
      scopeGlobal: "Todo el sistema",
      scopeHint:
        "Recomendado: «Todo el club» para que el miembro forme parte permanente del comité.",
      btnAddCommittee: "+ Agregar al comité",
      addCommitteeHint:
        "El usuario quedará con permiso «Comité de Handicap» en el alcance elegido. Después, en cada torneo, marca su asistencia con el botón verde de su tarjeta para habilitar su voto en esa sesión.",
      inviteByEmail: "¿No está en la lista? Invitar por email",
      userEmail: "Email del usuario",
      emailPh: "miembro@email.com",
      btnSearchAuthorize: "Buscar y autorizar",
      inviteRequires:
        "Requiere que el usuario ya exista en el sistema. Si no existe, créalo primero en",
      usersNewLabel: "Usuarios → Nuevo",
      noMembersWarning:
        "Aún no hay miembros autorizados. Usa el formulario de arriba o asigna el rol en",
      usersLink: "Usuarios",
      directorsAutoIncluded: ". Los directores del torneo aparecen automáticamente.",
      globalProgress: "Avance global del comité",
      votesShort: "votos",
      activeMembers: "Miembros activos:",
      invalidVotesTitle: "Votos no válidos (no entran al promedio)",
      invalidVotesHint:
        "Se conservan en la base. Solo cuentan miembros con rol handicap_committee marcados presentes.",
      invalidNoRole: "sin rol de comité",
      invalidNotPresent: "sin presencia",
      playersLabel: "Jugadores:",
      completed100: "Completaron 100%:",
      pendingMembers: "Pendientes:",
      scopeBadgeTournament: "Este torneo",
      scopeBadgeClub: "Todo el club",
      scopeBadgeGlobal: "Todo el sistema",
      director: "Director",
      present: "Presente",
      absent: "Ausente",
      votesDone: "Votos realizados",
      abstSuffix: "abst.",
      btnMarkAbsent: "🔕 Marcar ausente",
      btnMarkPresent: "✅ Marcar presente",
      btnRevoke: "Quitar",
      trimSectionTitle: "Recorte de outliers y umbral «No jugar»",
      removeHighVotes: "Quitar votos altos (más suaves)",
      removeLowVotes: "Quitar votos bajos (más severos)",
      minDqVotes: "Mínimo de votos «No jugar»",
      btnSaveRules: "Guardar reglas",
      trimExplain:
        "Recorte: por cada jugador se descartan los N votos más cercanos a 0 y los N votos más cercanos a −5; el promedio y el HP sugerido se recalculan con los votos vivos. El recorte no se reduce si hay pocos votos: si no sobrevive ninguno, el ajuste queda en 0.",
      thresholdExplain1:
        "Umbral «No jugar»: si un jugador acumula ese número (o más) de votos «No permitir jugar este torneo», queda marcado en rojo abajo.",
      thresholdExplain2: "= solo se muestra el conteo, sin marcar.",
      abstentionsInAverage: "Contar abstenciones como 0 en el promedio",
      abstentionsInAverageHint:
        "Desactivado (recomendado): el promedio solo usa ajustes numéricos que sobreviven al recorte. Activado: las abstenciones diluyen el promedio hacia 0.",
      aggregateHelp:
        "Resumen agregado (anonimizado): se muestran los votos individuales sin nombre. Verde = ajuste que entra al promedio, gris = descartado por recorte. Ajustes = numéricos que sobreviven al recorte (entran al promedio). Abst. = abstenciones (entran al promedio solo si está activada la opción del comité). Un jugador solo con abstenciones muestra 0 ajustes.",
      thPlayer: "Jugador",
      thHiCurrent: "HP torneo",
      thVotesAnon: "Votos (anónimos)",
      thLiveAvg: "Ajustes / Abst.",
      thLiveAvgTitle:
        "Ajustes = votos numéricos que entran al promedio (tras recorte). Abst. = abstenciones. El denominador del promedio respeta la opción del comité.",
      thAdjShort: "Aj.",
      thAbstShort: "Abst.",
      thAdjTitle: "Ajustes numéricos que sobreviven al recorte (entran al promedio)",
      thAbstTitle: "Abstenciones (no son ajustes; entran al promedio solo si está activado)",
      thAvgTrim: "Prom. recortado",
      thHiSug: "HP sugerido",
      thNoPlay: "No jugar",
      thRoundedAdj: "Ajuste HP",
      thRoundedAdjTitle: "Ajuste en golpes al handicap de torneo (enteros, −5 a 0)",
      thFinalHi: "HP definitivo",
      thFinalHiTitle: "Handicap de torneo que se aplicará (HP actual + ajuste)",
      applyHi: "Aplicar handicap del torneo",
      applyHiDone: "Ya aplicado",
      committeeAppliedNote: "Handicap del torneo ya aplicado (comité: {adj})",
      committeeAppliedHint: "Modifica el ajuste para volver a aplicar",
      applyAllSelected: "Aplicar a seleccionados",
      applyAllBtn: "Aplicar handicap del torneo a todos",
      bulkBarTitle: "Aplicar handicap del torneo",
      bulkBarHint:
        "Los ajustes son golpes directos al handicap de torneo (HP). Sin votos el ajuste inicia en 0 y puedes editarlo antes de aplicar.",
      bulkBarSelected: "seleccionados",
      bulkBarApplying: "Aplicando…",
      bulkBarSelectAll: "Seleccionar todos",
      bulkBarClear: "Quitar todo",
      manualAdjNoVotesHint:
        "Sin votos del comité: ajuste manual (inicia en 0).",
      noVotesYet: "—",
      noLiveVotes: "Sin votos vivos",
      ghinTitle: "GHIN del jugador",
      noVotes: "Sin votos",
      avgDivisor: "Prom.: ÷",
      abstAsZero: "abst. en promedio",
      chipAbstain: "Abstención (metadato; no diluye el promedio salvo que esté activado)",
      chipTrimmedLow: "Descartado (más severo)",
      chipTrimmedHigh: "Descartado (más suave)",
      chipLive: "Voto vivo",
      chipAbst: "0·abst",
      thresholdAuto: "Supera el umbral: jugador no autorizado",
      thresholdConfigured: "Umbral configurado:",
      thresholdInfo: "Solo informativo (umbral desactivado)",
      votesWord: "votos",
      notAuthorized: "No autorizado",
    },
    reset: {
      toggle: "Reiniciar votación (pruebas)",
      sessionName: "Nombre de esta sesión (opcional)",
      sessionNamePh: "Ej. Primera votación",
      notes: "Notas (opcional)",
      notesPh: "Motivo del reinicio",
      confirmLabel: "Escribe REINICIAR para confirmar",
      confirmPh: "REINICIAR",
      submit: "Archivar y borrar votos",
      archiving: "Archivando…",
      successArchived:
        "Sesión «{name}» archivada. {n} jugadores con votos, {m} votos borrados.",
      noVotes:
        "No hay votos para archivar. La votación ya está vacía; no se creó una sesión nueva.",
      hint: "Guarda un resumen anónimo de la votación actual en el historial y pone en cero los votos de todos los miembros del comité. Cada miembro deberá volver a calificar a todos los jugadores.",
    },
    history: {
      title: "Historial de votaciones",
      titleWithCount: "Historial de votaciones ({n})",
      empty:
        "Aún no hay sesiones archivadas. Al reiniciar la votación se guardará aquí un resumen anónimo de los resultados.",
      subtitle:
        "Sesiones guardadas al reiniciar. Los votos individuales permanecen anónimos.",
      sessionFallback: "Sesión",
      summaryVoters: "votantes",
      summaryPlayers: "jugadores",
      trimSummary: "Recorte: −{soft} suaves / −{hard} severos",
      vetoThreshold: "Umbral «No jugar»:",
      thresholdOff: "off",
      presentLabel: "Presentes:",
      notesLabel: "Notas:",
      thPlayer: "Jugador",
      thHi: "HP",
      thVotes: "Votos",
      thAvg: "Prom.",
      thHiSug: "HP sug.",
      thNoPlay: "No jugar",
      chipAbstention: "Abstención",
      chipTrimmed: "Descartado por recorte",
      chipLive: "Voto vivo",
      chipAbst: "0·abst",
      notAuthorized: " · No autorizado",
    },
  },
};

type Messages = typeof es;

const en: Messages = {
  language: {
    label: "Language",
    es: "ES",
    en: "EN",
    ariaEsActive: "Current language: Spanish",
    ariaEsInactive: "Switch interface to Spanish",
    ariaEnActive: "Current language: English",
    ariaEnInactive: "Switch interface to English",
  },
  nav: {
    enter: "Sign in",
    signOut: "Sign out",
    signOutTitle: "Sign out so another user can sign in",
  },
  sidebar: {
    activeTournament: "Active tournament",
    loading: "Loading...",
    noName: "Untitled",
    noTournament: "No tournament selected",
    modePrefix: "Mode",
    operation: "Operation",
    configShort: "Setup",
    configuration: "Configuration",
    setupSection: "Tournament configuration",
    publicPage: "Public page",
    switchTo: "Switch to",
    listTournaments: "Tournament list",
    brand: "List.golf",
    selectTournamentHint: "Select a tournament to see this menu.",
    openMenu: "Open navigation menu",
    closeMenu: "Close menu",
    nav: {
      dashboard: "Dashboard",
      players: "Players",
      entries: "Entries",
      auction: "Auction",
      teeSheet: "Starters / Groups",
      scoreEntry: "Score entry",
      scorecards: "Scorecards",
      scorecardsMp: "MP scorecards (print)",
      leaderboard: "Leaderboard",
      cercanos: "Closest to pin",
      cercanosPremios: "CTP prizes",
      caddies: "Caddies",
      ritmo: "Pace of play",
      seguimientoCaptura: "Lagging captures",
      reports: "Reports",
      tournaments: "Tournaments",
      editTournament: "Edit tournament",
      tournamentSetup: "Tournament setup",
      clubs: "Clubs",
      courses: "Courses",
      courseHolePoints: "Yardage points",
      categories: "Categories",
      rounds: "Rounds",
      teeSets: "Tee sets",
      tournamentHoles: "Tournament holes",
      cutRules: "Cut rules",
      competitionRules: "League",
      prizeRules: "Prizes",
      convocatoria: "Call / rules doc",
      categoryTemplates: "Category templates",
      users: "Users",
      handicapCommittee: "Handicap committee",
      committeeVote: "Voting",
      committeeSelection: "Player selection",
      committeeGhinData: "GHIN data",
      capturaTelegram: "Telegram mode",
      fbAdmin: "Restaurant",
      fbCocina: "Kitchen · Orders",
      fbCuentas: "Accounts · Billing",
      fbNuevoPedido: "New order (verbal)",
      fbReportes: "Daily reports",
      fbDisputas: "Disputes (committee)",
      fbMesero: "Waiter · Restaurant",
      fbInventario: "Inventory",
      fbFraccionamiento: "Neighborhood",
      fbCuentasDeposito: "Deposit accounts",
      fbRestaurante: "Restaurant profile",
      dailyRounds: "Daily rounds",
    },
  },
  common: {
    tournament: "TOURNAMENT",
    change: "Change",
    home: "← Home",
    noCategory: "No category",
  },
  publicHome: {
    title: "Public tournaments",
    subtitle: "Posters and links to each tournament’s public page.",
    labelScope: "Scope",
    labelCity: "City",
    labelClub: "Club",
    scopeAll: "All",
    scopeActive: "Active tournaments",
    scopeFuture: "Upcoming tournaments",
    scopeHistoric: "Past tournaments",
    cityAll: "All cities",
    cityUnknown: "No city",
    clubAll: "All clubs",
    apply: "Apply",
    clear: "Clear",
    countNone: "0 tournaments",
    countOne: "1 tournament",
    countMany: "{n} tournaments",
    empty: "No tournaments match these filters.",
    noPoster: "No poster",
    noClub: "No club",
    statusTbd: "TBD",
    statusUpcoming: "Upcoming",
    statusActive: "Active",
    statusHistoric: "Past",
    scopeHint:
      "Active: started and not closed yet (uses end date when set; otherwise up to 4 weeks after the start). Upcoming: start after today. Past: already closed (end before today; without an end date, after 4 weeks from the start).",
    loadErrorTitle: "We couldn’t load the tournament list",
    loadErrorBody:
      "This may be Supabase RLS, a missing column in production versus the app (migrations not applied), or another PostgREST error. Check the technical detail below and Vercel runtime logs.",
    loadErrorTechnical: "Server detail",
  },
  dashboard: {
    title: "Dashboard",
    kpiActive: "Active tournaments",
    kpiPlayers: "Total players",
    kpiLive: "Live scorecards",
  },
  reports: {
    title: "Reports",
  },
  players: {
    title: "Players",
  },
  leaderboard: {
    title: "Leaderboard",
  },
  entries: {
    title: "Entries",
    list: {
      heading: "Registered players",
      searchPlaceholder: "#, name, club, status or round...",
      optionClub: "Club",
      optionCat: "Cat",
      thNumber: "#",
      thPlayer: "Player",
      thClub: "Club",
      thHcp: "Hcp",
      thHcp80: "HCP 80%",
      thHcp80Title:
        "Official tournament handicap at 80%: HI × (Slope/113) + (CR − Par), then × 0.80. Shows HI, tee, slope, CR, par and CH.",
      thCat: "Cat",
      thStatus: "Status",
      thSignatures: "Signatures",
      thActions: "Actions",
      statusActive: "Active",
      statusWithdrawn: "WD",
      statusDQ: "DQ",
      noResults: "No results",
      btnSignatures: "SIGN",
      btnDelete: "DELETE",
      deletePending: "Deleting...",
      confirmDelete:
        "Delete permanently? Only allowed if no hole scores are captured.",
      btnRea: "REA",
      restorePending: "Restoring...",
      confirmRestoreWithdrawn: "Reactivate this player in the tournament?",
      confirmRestoreDq: "Remove DQ and return the player to active?",
      btnWithdraw: "WD",
      withdrawPending: "Withdrawing...",
      confirmWithdraw: "Withdraw this player from the tournament?",
      btnDq: "DQ",
      dqPending: "Setting DQ...",
      confirmDq:
        "Mark DQ? This will set 400 for the current/last round and mark the player DQ.",
      alertNoRoundId: "round_id was not found in the URL.",
      linksCopied: "Links copied to clipboard",
      linksError: "Error generating links",
      linksPlayer: "Player:",
      linksMarker: "Marker:",
      linksWitness: "Witness:",
      roundSigTitle: "R{round}: {count} signature(s)",
      roundBallCaptured: "R{round}: captured",
      roundBallCapturedOpen: "R{round}: captured, not closed",
      roundBallPending: "R{round}: not captured",
      roundBallClosed: "R{round}: closed",
      roundBallLegend:
        "Red: not captured · Amber: captured, not closed · Green: scorecard closed",
      btnTelegramKit: "KIT",
      btnTelegramKitTitle: "Telegram setup for this player",
    },
    telegramKit: {
      title: "Telegram connection kit",
      backToEntries: "← Back to entries",
      playerLabel: "Player",
      tournamentLabel: "Tournament",
      botUsernameLabel: "Tournament Telegram bot",
      botUsernameHint:
        "Tell the player to find this username in Telegram, tap Start, and send HOLA. Copy the @ to read it aloud.",
      copyBotUsername: "Copy @username",
      copied: "Copied",
      staffScriptTitle: "What to tell the player",
      staffScript:
        "«Open Telegram, search for @{bot}, tap Start, and send HOLA. If the bot recognizes you, we're linked; if not, copy your numeric ID and we'll save it here.»",
      phaseConnect: "Step 1 — Connect Telegram",
      phaseVerify: "Step 2 — Verify link",
      phaseDeliver: "Step 3 — Deliver kit",
      statusLinked: "Telegram linked on file (user ID saved).",
      statusNotLinked: "Telegram not linked yet.",
      statusKitSent: "Kit sent on Telegram; waiting for player to reply RECIBIDO.",
      statusKitReceived: "Player confirmed kit received.",
      step1Title: "1. Open the bot in Telegram",
      step1BodyConfigured:
        "The player should open the bot using the username above, tap Start, and send HOLA.",
      step1BodyNotConfigured:
        "Set NEXT_PUBLIC_TELEGRAM_BOT_USERNAME on the server (e.g. @ListGolfBot).",
      openBotLink: "Open bot in Telegram",
      step2Title: "2. Get the ID if the bot does not recognize the player",
      step2Body:
        "Ask them to send HOLA or ID to the tournament bot. The bot replies with a number between ━━━━ lines. You can also use @userinfobot. Recent unlinked contacts appear below.",
      pendingLinksTitle: "Recent bot messages (unlinked)",
      pendingLinksHint:
        "If they already sent HOLA, find their name here and tap «Use this ID».",
      pendingLinksEmpty:
        "No recent unlinked contacts. Have the player send HOLA or ID to the bot.",
      pendingLinksUseId: "Use this ID",
      openUserinfobot: "Open @userinfobot",
      step3Title: "3. Save ID on profile",
      step3Body: "Saving the ID lets the bot recognize this player from Telegram.",
      verifyTitle: "Verify the link works",
      verifyBody:
        "Send a test message to the player's Telegram. If they receive it, the link is correct.",
      btnVerify: "Send verification via Telegram",
      verified: "Verification message sent.",
      deliverTitle: "Deliver tournament kit",
      deliverBody:
        "Send the kit contents on Telegram and ask the player to reply RECIBIDO when done.",
      kitPreviewLabel: "Message preview",
      kitExtraNote: "Extra note for the player (optional)",
      btnDeliverKit: "Send kit via Telegram",
      kitSent: "Kit sent. Wait for the player to message RECIBIDO to the bot.",
      formTitle: "Save ID on player profile",
      fieldUserId: "Telegram user ID (digits only)",
      fieldChatId: "Chat ID (optional)",
      fieldChatHint:
        "Often matches the user id in private chats; leave empty if unsure.",
      btnSave: "Save to profile",
      btnClear: "Remove Telegram link",
      saving: "Saving…",
      missingParams: "Tournament or player is missing from the link.",
      notEnrolled: "This player is not entered in the selected tournament.",
      invalidUserId: "User ID must be digits only.",
      saveError: "Could not save",
      saved: "Saved successfully.",
      partialDeliveryLabel: "Partial delivery (player still owes kit items)",
      partialPendingPlaceholder: "E.g. credential, hat, printed rules…",
      partialPendingHint:
        "If checked, the player can reply RECIBIDO PARCIAL then RECIBIDO when complete.",
      statusKitPartial: "Kit partially received.",
      linkKitContent: "Edit tournament kit content",
      statusKitPendingItems: "Still to deliver:",
    },
    telegramKitContent: {
      title: "Telegram kit content",
      backEntries: "← Entries",
      tournament: "Tournament",
      loadTournament: "Load",
      editing: "Editing",
      greeting: "Greeting",
      body: "Message body",
      footer: "Closing instructions (RECIBIDO / partial)",
      placeholders:
        "Variables: {player_name}, {tournament_name}. Use line breaks in the body.",
      save: "Save template",
      saved: "Template saved.",
      noTournaments: "No tournaments.",
    },
    bulk: {
      title: "Bulk entry",
      searchPlaceholder: "Search name or club...",
      allClubs: "All clubs",
      allCats: "All cat.",
      selectVisibleBusy: "Selecting...",
      selectVisible: "Select visible",
      clearVisibleBusy: "Clearing...",
      clearVisible: "Clear visible",
      visibleCount: "Visible:",
      selectedCount: "Selected:",
      selectedVisibleCount: "Selected (visible):",
      thSel: "Sel",
      thPlayer: "Player",
      thClub: "Club",
      thHi: "HI",
      thCat: "Cat",
      emptyPlayers: "No players to show",
      enrolling: "Enrolling...",
      enrollSelected: "Enroll selected ({n})",
    },
    summary: {
      title: "Tournament summary",
      players: "Players:",
      categories: "Categories:",
      clubs: "Clubs:",
      byCategory: "Players by category",
      thCategory: "Category",
      thEnrolled: "Entered",
      thQuota: "Quota",
      thState: "State",
      full: "FULL",
      spotsFree: "{n} open",
      noLimit: "No limit",
      noData: "No data",
      byClub: "Players by club",
      thClub: "Club",
      thPlayers: "Players",
      noClub: "No club",
    },
    single: {
      title: "Enter player",
      searchPlaceholder: "Search name or club...",
      newPlayer: "New player",
      thPlayer: "Player",
      thClub: "Club",
      thHi: "HI",
      thAge: "Age",
      thCategory: "Category",
      thAction: "Action",
      categoryNormal: "Standard category",
      unnamed: "Unnamed",
      auto: "Auto",
      noHi: "No HI",
      enrolling: "Enrolling...",
      enroll: "Enroll",
      emptySearch: "No player found in the master list.",
    },
    excel: {
      import: "Import from Excel",
    },
    bulkSelector: {
      title: "Bulk player entry",
      searchPlaceholder: "Search player...",
      allCategories: "All categories",
      allClubs: "All clubs",
      selectVisible: "Select visible",
      thSel: "Sel",
      thPlayer: "Player",
      thClub: "Club",
      thHi: "HI",
      thCategory: "Category",
      enrollSelected: "Enroll selected ({n})",
    },
    page: {
      tournamentLabel: "Tournament",
      registrationBlocked:
        "Registration, category/handicap edits, and removing players are blocked.",
      registrationOpen:
        "You can enroll players, edit category/handicap, and remove entries.",
      closeRegistration: "Close registration",
      bulkClosedNote:
        "Registration is closed. Reopen the tournament to use bulk entry.",
      load: "Load",
      registrationClosedTitle: "Registration closed",
      registrationOpenTitle: "Registration open",
      closedAtPrefix: "Closed:",
      reopenPlaceholder: "Reason to reopen",
      reopenButton: "Reopen registration",
      closePlaceholder: "Reason for closing",
      tabManual: "Manual",
      tabBulk: "Bulk",
      tabEntries: "Entered",
      tabSummary: "Summary",
      manualClosedMsg:
        "Registration is closed. Reopen the tournament to enter players manually.",
      bulkFeedbackSelected: "Selected:",
      bulkFeedbackAdded: "Added:",
      bulkFeedbackSkipped: "Already entered:",
      noTournamentSelected: "No tournament selected.",
      loadErrorTitle: "Could not load entries",
      loadErrorBody:
        "An error occurred while loading entries, scorecards, or scores. Try another tournament or reload.",
      loadErrorTechnical: "Technical detail",
    },
  },
  teeSheet: {
    title: "Starters / Groups",
    loadErrorTitle: "Could not load tee sheet",
    loadErrorBody:
      "This may be Supabase RLS, a missing column in production (pending migrations), or another PostgREST error. See the technical detail below.",
    loadErrorTechnical: "Server detail",
    registrationOpenGate:
      "Registration is still open. Close registration before generating tee sheet groups (R1, R2…).",
    priorRoundGate:
      "Cannot generate round {round} tee sheet until round {prior} is closed for: {categories}.",
    priorRoundGateAction:
      "Close the previous round for every active player in that category (official view or score entry).",
    standingsOrderHint:
      "Order from standings. A real cut applies only if Cut rules include a rule targeting R{round} («To round»). Otherwise everyone in the block tees off.",
    pairingScoreHcp: "HCP",
    pairingScoreR1: "R1",
    pairingScoreR1R2: "R1+R2",
    cutEnforcesForRound:
      "R{round}: real cut when generating groups (rule targets R{round}). Only players above the cut line in standings are included.",
    cutDoesNotEnforceForRound:
      "R{round}: no real cut for tee groups (no rule targets R{round}). All active entries in the block are included.",
    shotgunNoDoubleTees:
      "With {groups} groups in this block there are only A tees per hole. B tees (1B, 2B…) start at group 19.",
  },
  rounds: {
    title: "Rounds",
    subtitle:
      "Set up the tournament calendar by day, round, category, wave, and tee time.",
    newTournament: "+ New tournament",
    cutRulesLink: "Cut rules",
    formSaving: "Saving...",
    formDeleting: "Deleting...",
    btnDelete: "Delete",
    confirmDeleteRound:
      "Delete this round? If it has related groups or scores, the database may block deletion.",
  },
  teeSets: {
    title: "Tee sets",
    teeRulesLink: "Tee assignment rules",
  },
  courses: {
    title: "Golf courses",
    shortNameLabel: "Short name",
    shortNamePlaceholderCourse: "Abbr. e.g. CCQ",
    shortNamePlaceholderNewClub: "Club abbreviation",
    teeSetsSection: "Course tee sets",
    teeSetsHint:
      "Enter official WHS slope and course rating. Men and women may differ on the same tee (e.g. White tees).",
    coursesListTitle: "Registered courses",
    coursesListHint:
      "Open any course to review or capture slope, rating and WHS tee data.",
    thClub: "Club",
    thCourse: "Course",
    thTees: "Tees",
    thWhsStatus: "WHS",
    thAction: "",
    whsComplete: "Complete",
    whsPartial: "Partial",
    whsEmpty: "Empty",
    btnOpen: "Open",
    btnSelected: "Selected",
    coursesEmpty: "No courses registered yet.",
    thOrder: "ORDER",
    thCode: "CODE",
    thName: "NAME",
    thColor: "COLOR",
    thGender: "GEN.",
    thRatingMen: "RATING M",
    thSlopeMen: "SLOPE M",
    thRatingWomen: "RATING F",
    thSlopeWomen: "SLOPE F",
    genderEmpty: "—",
    genderM: "M",
    genderF: "F",
    genderX: "Mixed",
    saveTeeSets: "Save tee sets",
    whsColumnsMissing:
      "WHS columns (slope/rating) are not in the database yet. Apply migration 20260525210000_course_tee_sets_whs.sql in Supabase.",
  },
  clubs: {
    title: "Clubs",
    subtitle:
      "Master club directory, official logos, and auto-generated logos",
    searchPlaceholder: "Search club...",
    placeholderShortName: "Short name",
    thShortName: "Short name",
    sectionCreateTitle: "New club",
    placeholderClubName: "Club name",
    placeholderLogoUrlOptional: "Optional official logo URL",
    labelActive: "Active",
    labelInactive: "Inactive",
    saving: "Saving...",
    btnCreateClub: "Create club",
    sectionMergeTitle: "Merge duplicate club",
    sectionMergeDesc:
      "Moves courses and players from the duplicate club to the target club and marks the duplicate inactive. Does not delete the source record.",
    mergeSelectSource: "Duplicate / source club",
    mergeSelectTarget: "Primary / target club",
    merging: "Merging...",
    btnMerge: "Merge",
    clubOptionWithCourses: "{name} · courses: {count}",
    thLogo: "Logo",
    thClub: "Club",
    thCourses: "Courses",
    thOfficialLogo: "Official logo",
    thStatus: "Status",
    thActions: "Actions",
    emptyResults: "No results.",
    labelColor: "Color:",
    placeholderPasteOfficialLogoUrl: "Paste official logo URL",
    btnSaveLogo: "Save logo",
    cancel: "Cancel",
    badgeOfficial: "Official",
    badgeAutomatic: "Auto",
    hintUseShortNameInitials: "Use initials from the short name.",
    previewOfficialLogo: "Official logo",
    previewAutomaticLogo: "Auto logo",
    btnSave: "Save",
    btnEdit: "Edit",
    btnLogo: "Logo",
    btnRegenerate: "Regenerate",
    deactivate: "Deactivate",
    activate: "Activate",
    dash: "—",
    errCreateNotConfirmed: "Club creation was not confirmed.",
    errUpdateNotConfirmed: "Club update was not confirmed.",
    errShortNameMismatch:
      "The database did not return the expected short name. Save may have failed.",
    errLogoUpdateNotConfirmed: "Logo update was not confirmed.",
    errToggleNotConfirmed: "Status change was not confirmed.",
    errClubNotFoundEdit: "Club to edit was not found.",
    errClubNotFoundLogo: "Club for logo edit was not found.",
    errMergeSelectBoth: "Select duplicate club and target club.",
    errMergeSame: "Duplicate and target club cannot be the same.",
    errGenericCreateClub: "Error creating club",
    errGenericUpdateClub: "Error updating club",
    errGenericUpdateLogo: "Error updating logo",
    errGenericRegenerateLogo: "Error regenerating logo",
    errGenericToggle: "Error changing status",
    errGenericMerge: "Error merging clubs",
    infoClubCreated: "Club created successfully.",
    infoClubUpdated: "Club updated successfully.",
    infoLogoUpdated: "Logo updated successfully.",
    infoLogoRegenerated: "Auto logo regenerated.",
    infoClubDeactivated: "Club deactivated successfully.",
    infoClubActivated: "Club activated successfully.",
    infoMergeDone:
      "Merge complete. Courses and players were moved to the target club and the duplicate was marked inactive.",
  },
  scoreEntry: {
    title: "Score entry",
    subtitle:
      "Quick entry by registration number or name of a player enrolled in the tournament.",
    subtitleCapture:
      "Fast entry for the player's open round. Search by registration number or name.",
    subtitleModify:
      "Fix closed or prior rounds. Pick the round, OPEN, edit, and close again.",
    tabCapture: "Capture",
    tabModify: "Edit scores",
    tabNavAria: "Score entry mode",
    searchHintCapture:
      "Open round only. To fix a closed round use the Edit scores tab.",
    searchHintModify:
      "Pick the round above. For closed rounds: OPEN → edit → Close again.",
    captureClosedBanner:
      "This round is already closed. Daily capture only works on the open round.",
    captureClosedLink: "Go to Edit scores →",
    captureNextRoundLink: "Capture R{round} (next round)",
    labelSession: "Day & wave",
    roundsNeedDate:
      "Some rounds have no date. Assign a date to each round in tournament setup to use score entry.",
    searchAmbiguousTitle:
      "Several players match your search. Pick the correct enrollment:",
    searchNotFound: "No enrolled player matches \"{q}\".",
    priorRoundGate:
      "Round {round} score entry is blocked: round {prior} must be closed for all active players in category {category}.",
    registrationOpenGate:
      "Registration is still open. Close registration before capturing scorecards.",
    labelTournament: "Tournament",
    selectTournament: "Select a tournament",
    selectTournamentHint:
      "Choose the tournament before searching for a player so rounds and scores match the right event.",
    roundSwitcherKicker: "Round to enter or correct",
    roundCurrent: "On screen",
    roundClosed: "Closed · tap OPEN below",
    roundOpen: "Has scores",
    roundEmpty: "No scores yet",
    misalignedCapture:
      "Scores exist on a different round row for this tournament. Pick the correct round above or use «Repair captures» (directors).",
    playerFoundHint:
      "To fix R1 while R2 is in progress: select «R1» above, then «OPEN», edit, and save.",
  },
  scorecards: {
    title: "Live scorecard (Supabase test)",
    subtitle: "Full digital signing: marker + player + witness.",
  },
  caddies: {
    title: "Caddies",
    subtitle: "Operational caddie assignments by tournament and round",
    catalog: "New / Directory",
    tournaments: "Tournaments",
    entries: "Entries",
    scoreEntry: "Score entry",
    filters: "FILTERS",
    filtersHint: "Filter assignments by tournament and round",
    assignHeading: "ASSIGN CADDIE BY ROUND",
    assignments: "ASSIGNMENTS",
    telegramLink: {
      title: "Caddie Telegram",
      migrationWarning:
        "Missing Supabase migration: telegram_user_id and telegram_chat_id on caddies. Run 20260527160000_caddies_telegram_link.sql.",
      statusLinked: "Telegram linked (user ID saved).",
      statusNotLinked: "Telegram not linked yet.",
      step1Title: "1. Open the bot in Telegram",
      step1BodyConfigured:
        "The caddie should open the bot, tap Start, and write HOLA.",
      step1BodyNotConfigured:
        "Set NEXT_PUBLIC_TELEGRAM_BOT_USERNAME on the server.",
      openBotLink: "Open bot in Telegram",
      botUsernameLabel: "Tournament bot",
      copyBotUsername: "Copy @username",
      copied: "Copied",
      step2Title: "2. Get the ID if the bot does not recognize them",
      step2Body:
        "Ask them to write HOLA or ID to the bot. The bot replies with a number between ━━━━ lines. Unlinked recent writers appear below.",
      pendingLinksTitle: "Recent bot messages (unlinked)",
      pendingLinksHint:
        "If the caddie already wrote HOLA, find their name and tap «Use this ID».",
      pendingLinksEmpty: "No unlinked users have messaged the bot recently.",
      pendingLinksUseId: "Use this ID",
      step3Title: "3. Save ID on profile",
      step3Body:
        "Save the ID so the bot can identify them and receive group capture links.",
      formTitle: "Register ID on caddie profile",
      fieldUserId: "Telegram user ID (digits only)",
      fieldChatId: "Chat ID (optional)",
      fieldChatHint:
        "Usually matches user id in private chats; leave empty if unsure.",
      btnSave: "Save link",
      btnClear: "Remove Telegram link",
      verifyTitle: "Verify link",
      verifyBody: "Send a test message to the caddie's Telegram.",
      btnVerify: "Send verification via Telegram",
      savedBanner: "Link saved successfully.",
      verifiedBanner: "Verification message sent.",
      editTelegram: "Link Telegram",
    },
  },
  tournamentHoles: {
    title: "Tournament holes",
  },
  categories: {
    title: "Categories",
  },
  cutRules: {
    title: "Cut rules",
    noTournaments:
      "No tournaments yet. Create a tournament first to configure cut rules.",
    linkTeeRules: "Tee assignment rules",
    linkCompetition: "Competition",
    savedOk: "Rules saved successfully.",
    editor: {
      prerequisiteBanner:
        "Create rounds and categories in this tournament before configuring cuts.",
      newRule: "New rule",
      helperLine:
        "Cuts per category: exact field size (percent rounded down on entries). At the cut line the tie-break profile applies (never all tied players).",
      thOrder: "Order",
      thCategories: "Categories",
      thFrom: "From",
      thTo: "To",
      thBasis: "Basis",
      thMode: "Mode",
      thType: "Type",
      thCut: "Cut",
      thTies: "Ties",
      thGrossProtection: "Gross protection",
      thTieBreak: "Tie-break",
      thActive: "Active",
      thNotes: "Notes",
      thActions: "Actions",
      noRulesYet: "No rules yet.",
      selectedPrefix: "Selected:",
      pickOneCategory: "Select at least one category",
      passAll: "All pass",
      topGross: "Top gross",
      noExemption: "No",
      noTieBreak: "No tie-break",
      titleMoveUp: "Move up",
      titleMoveDown: "Move down",
      delete: "Delete",
      saveRules: "Save rules",
      observationsPlaceholder: "Notes",
      roundOptionPrefix: "R",
      addRowNeed: "Create rounds and categories first to define rules.",
      valNeedRound: "Create at least one round first.",
      valNeedCategories: "Create categories first.",
      valFromRoundInvalid: "Invalid from round in row {row}.",
      valToRoundInvalid: "Invalid to round in row {row}.",
      valToRoundOrder:
        "To round must be greater than or equal to from round in row {row}.",
      valPickCategory: "Select at least one category in row {row}.",
      valCutValueInvalid: "Invalid cut value in row {row}.",
      valTopN: "Top N must be greater than or equal to 1 in row {row}.",
      valTopPercent: "Top % must be between 0 and 100 in row {row}.",
      valGrossExempt:
        "Protected gross top must be greater than or equal to 1 in row {row}.",
      valNeedTieBreak:
        "Select a tie-break profile in row {row} (required for exact cut size).",
      valCutScopeGroup:
        "Category group must be at least 2 letters or an exact code (e.g. DE), not a single letter like “D” (row {row}).",
      rankingBasisLabels: {
        gross_total: "Gross total",
        net_total: "Net total",
        points_total: "Points total",
        gross_round: "Gross round",
        net_round: "Net round",
        points_round: "Points round",
      },
      rankingModeLabels: {
        tournament_to_date: "Tournament to date",
        specified_rounds: "From / to range",
        last_round_only: "Last round only",
      },
      advancementLabels: {
        top_n: "Top N",
        top_percent: "Top %",
        all: "No cut / all advance",
      },
    },
  },
  categoryTemplates: {
    title: "Category templates",
    headerTemplate: "TEMPLATE",
    headerNew: "NEW TEMPLATE",
  },
  categoryTeeRules: {
    title: "Tee rules by category",
    linkTeeSets: "Tee sets",
  },
  competitionRules: {
    title: "Competition rules",
    noTournaments:
      "No tournaments yet. Create a tournament first to configure competition rules.",
    linkCuts: "Cuts",
    linkPrizes: "Prizes",
    linkCompetition: "Competition",
    savedOk: "Rules saved successfully.",
    editor: {
      unnamedCategory: "Unnamed",
      basisGross: "Gross",
      basisNet: "Net",
      basisBoth: "Both",
      basisStableford: "Stableford",
      scoringStrokePlay: "Stroke play",
      scoringStableford: "Stableford",
      netPlacesAll: "All",
      valMissingTournamentId: "Missing tournament_id.",
      valNoCategories: "This tournament has no categories to configure.",
      valMissingCategoryId: "Missing category_id in {label}.",
      valHandicapNotNumber: "Invalid handicap % in {label}.",
      valHandicapRange: "Handicap % must be between 0 and 150 in {label}.",
      valGrossPlacesInvalid: "Invalid gross places in {label}.",
      valGrossPlacesNegative: "Gross places cannot be negative in {label}.",
      valNetPlacesInvalid: "Invalid net places in {label}.",
      valNetPlacesNegative: "Net places cannot be negative in {label}.",
      valSfLbMismatch:
        "If {label} is Stableford, the leaderboard must be Stableford.",
      valSfPrizeMismatch:
        "If {label} is Stableford, prizes must be Stableford.",
      valSpLbSf:
        "If {label} is stroke play, the leaderboard cannot be Stableford.",
      valSpPrizeSf: "If {label} is stroke play, prizes cannot be Stableford.",
      rowFallback: "row {n}",
      introBlurb:
        "Set the format per category before calculating cuts, leaderboard, and prizes.",
      badgeCategories: "Categories:",
      badgeActive: "Active:",
      badgeStableford: "Stableford:",
      badgeHcp80: "HCP 80%:",
      filterPlaceholder: "Search category, format, or basis",
      netEmptyHint:
        "Empty net = pay everyone else after gross places.",
      thOrder: "Order",
      thCategory: "Category",
      thFormat: "Format",
      thPrizes: "Prizes",
      thHcpPct: "% HCP",
      thGross: "Gross",
      thNet: "Net",
      thActive: "Active",
      thNotes: "Notes",
      thSummary: "Summary",
      noFilterMatch: "No categories match the filter.",
      observationsPlaceholder: "Notes",
      summaryLbPrefix: "LB",
      summaryPrizesHandicap: "Prizes {prizeBasis} · HCP {pct}%",
      summaryGrossNet: "Gross {gross} · Net {net}",
      saveButton: "Save competition rules",
      saveFooterNote:
        "All categories for the tournament are saved so leaderboard and prizes read a single source.",
    },
  },
  convocatoria: {
    title: "Tournament call (convocatoria)",
    intro:
      "Build the call in-app from the 68th Annual template, edit all fields, close it, then generate tournament parameters.",
    noTournaments: "Create a tournament first.",
    newTournament: "New tournament",
    linkPrizes: "Prizes",
    startFromTemplate: "Start from template",
    templateHint:
      "68th Annual CCQ template: categories, 50% cut, competition rules, podium prizes. Fully editable.",
    loadTemplateButton: "Reload 68th Annual template (CCQ)",
    autoProvisionNote:
      "68th Annual CCQ template loaded automatically. Review tabs and save when ready.",
    autoSyncNote:
      "Cuts and prizes synced to 68th Annual call. Review and save the draft.",
    rulesUpgradedNote:
      "Cut rules in DB updated: 36-hole total (R1+R2) and CCQ tie-break profiles.",
    reloadTemplateHint:
      "Replaces the editing draft with the template (discards unsaved DB changes).",
    importOptionalTitle: "Import Word (optional)",
    importOptionalHint: "Only if you have a different .docx; replaces draft while editing.",
    importOptionalButton: "Import .docx",
    statusEditing: "Editing",
    statusClosed: "Closed — ready to generate parameters",
    statusApplied: "Parameters already applied",
    tabMeta: "General",
    tabReference: "Call text",
    tabCategories: "Categories",
    tabCompetition: "Competition",
    tabCuts: "Cuts",
    tabPrizes: "Prizes",
    saveDraft: "Save draft",
    closeConvocatoria: "Close call document",
    reopenConvocatoria: "Reopen for editing",
    generateParams: "Generate tournament parameters",
    generateBlocked: "Entries exist: cannot auto-replace.",
    confirmClose: "Closing locks editing until you reopen.",
    confirmGenerate: "Writes categories, competition, cuts, prizes, and rounds.",
    closedOk: "Call closed. Review and click Generate tournament parameters.",
    appliedOk: "Parameters generated. Review tournament holes and tee sets.",
    appliedOkMatchPlay:
      "Match play parameters generated. Review categories and pair entries next.",
    introMatchPlay:
      "Build the match-play pairs call: format, bracket, categories, and tiebreakers.",
    autoProvisionNoteMatchPlay:
      "Match play template loaded automatically. Review rules and categories.",
    linkMatchPlay: "Match play",
    tabMatchPlayRules: "Match play rules",
    matchPlayBadge: "Match Play",
    confirmGenerateMatchPlay:
      "Creates categories, prizes, bracket rounds, and match play rules.",
    readOnlyHint: "Read-only while closed or applied",
    metaTitle: "Title",
    metaHoles: "Total holes",
    metaCutHoles: "Cut after (holes)",
    metaCutPct: "Max cut %",
    metaRounds: "Rounds",
    metaPracticeDay: "Practice day",
    metaHandicapDate: "Handicap index date",
    refSystem: "Playing system",
    refGentlemen: "Gentlemen",
    refLadies: "Ladies",
    refSeniorsAges: "Senior ages",
    refCutPolicy: "Cut policy",
    refCutTiebreakGross: "Cut tiebreak (gross)",
    refCutTiebreakStableford: "Cut tiebreak (Stableford)",
    refCutTiebreakSeniors: "Cut tiebreak (seniors)",
    refTrophyTiebreak: "Trophy tiebreak",
    refTrophies: "Trophies & special prizes",
    refOutOfScope: "Not auto-generated",
    reviewWarnings: "Warnings",
    colCode: "Code",
    colName: "Name",
    colHcp: "H.I.",
    colCut: "Cut",
    colGroup: "Group",
    colGender: "Gender",
    addCategory: "Add category",
    migrationMissing:
      "Table tournament_convocatoria is missing. Run migrations 20260518120000 and 20260518130000 in Supabase first.",
  },
  prizeRules: {
    title: "Prize rules",
    linkCut: "Cut rules",
    linkLeaderboard: "Leaderboard",
    noTournaments:
      "No tournaments yet. Create a tournament first to configure prizes.",
  },
  users: {
    title: "USERS",
  },
  tournaments: {
    title: "TOURNAMENTS",
    colEntries: "Entries",
    subtitle: "Master operations control",
    linkPublicHome: "Public home",
    linkPublicTournaments: "Public tournaments",
    newTournament: "New tournament",
    searchHeading: "SEARCH / SORT",
    searchHint:
      "Filter by host club (only clubs with at least one tournament) and by start date range.",
    labelClub: "Host club",
    clubSelectAll: "All clubs",
    labelDateFrom: "From date",
    labelDateTo: "To date",
    filter: "Filter",
    clear: "Clear",
    allByDate: "All by date",
    filterShowing: "Showing",
    tournamentOne: "tournament",
    tournamentMany: "tournaments",
    filteredOne: "filtered",
    filteredMany: "filtered",
    byClub: "by club",
    wordFrom: "from",
    wordTo: "to",
    orderPrefix: "Sort:",
    orderNewestFirst: "newest to oldest",
    filterAllIntro: "Showing",
    filterAllTournaments: "all tournaments",
    filterAllOrdered: "sorted",
    thDate: "Date",
    thTournament: "Tournament",
    thStatus: "Status",
    thPublic: "Public",
    thClub: "Club",
    thCourse: "Course",
    thHoles: "Holes",
    thCategories: "Categories",
    thTeeSets: "Starters",
    thRules: "Tee rules",
    thRounds: "Rounds",
    thStaff: "Staff",
    thActions: "Actions",
    thConvocatoria: "Call",
    thCompetition: "Competition",
    thCuts: "Cuts",
    thPrizes: "Prizes",
    noPoster: "No poster",
    clubLine: "Club:",
    courseLine: "Course:",
    btnPublic: "Public",
    edit: "Edit",
    scores: "Scores",
    hide: "Hide from public site",
    show: "Publish on public site",
    showArchivedHint: "Reactivate the tournament before publishing.",
    reactivate: "Reactivate",
    archive: "Archive",
    updating: "Updating...",
    emptyWithFilters: "No tournaments match these filters.",
    visibility: {
      archived: "Archived",
      public: "Public",
      hidden: "Hidden",
    },
  },
  courseHoles: {
    title: "Course scorecard",
  },
  tournamentSetup: {
    courseWithoutName: "Unnamed course",
    unnamed: "Unnamed",
    noCategoriesLoaded: "No categories loaded",
    noRoundsLoaded: "No rounds loaded",
    pageTitle: "Tournament setup",
    pageSubtitle:
      "Existing tournaments · generate fields, apply templates, and prepare operations.",
    labelTournament: "Tournament",
    selectTournament: "Select tournament",
    change: "Change",
    newTournament: "New tournament",
    summaryTitle: "Tournament summary",
    linkCategories: "Categories",
    linkStarters: "Starters",
    linkRounds: "Rounds",
    linkScoreEntry: "Score entry",
    linkHandicapCommittee: "Handicap committee",
    infoTournament: "Tournament",
    infoClub: "Club",
    infoCourse: "Course",
    infoAbbr: "Abbr.",
    infoStart: "Start",
    infoCategories: "Categories",
    quickInitTitle: "Quick initialization",
    quickInitSubtitle:
      "Generate tournament course, categories from a template, and base rounds in one action.",
    currentCourse: "Current course",
    currentCategories: "Current categories",
    currentRounds: "Current rounds",
    labelClub: "Club",
    all: "All",
    viewCourses: "View courses",
    baseCourse: "Base course",
    changeBaseCourse: "Change base course",
    selectCourse: "Select course",
    modifyCategories: "Change categories",
    leaveCurrentCategories: "Keep current categories",
    selectTemplate: "Select template",
    replaceCurrentCategories: "Replace current categories",
    roundsShort: "Rounds",
    initTournament: "Initialize tournament",
    initWarning:
      "Replaces tournament_holes and rounds. If you pick a template, it also replaces categories. If the tournament has no start date, rounds are created using today’s date.",
    sectionCourse: "Course",
    sectionCourseDesc:
      "Pick club and base course to generate the tournament card in tournament_holes.",
    generateCourseFields: "Generate tournament fields",
    replaceHolesOnly:
      "This replaces current records in tournament_holes.",
    thHole: "HOLE",
    thPar: "PAR",
    thHandicap: "HCP",
    noCourseGenerated: "This tournament has no generated fields yet.",
    sectionCategories: "Categories",
    sectionCategoriesDesc: "Apply an existing template to the current tournament.",
    applyTemplate: "Apply template",
    currentCategoriesPrefix: "Current categories:",
    deleteCategoriesWarning:
      "This first deletes the tournament’s current categories, then loads those from the selected template.",
    noActiveTemplates: "No active templates available.",
    fineTuneHint: "You can fine-tune details in the categories module.",
    sectionStarters: "Starters",
    sectionStartersDesc: "Jump to starters and rules for this tournament.",
    openStarters: "Open starters",
    teeRules: "Tee assignment rules",
    sectionLeagueConvocatoria: "League and call document",
    sectionLeagueConvocatoriaDesc:
      "Build the tournament call (template, cuts, prizes, rules) or edit league and prize settings per category.",
    linkConvocatoria: "Tournament call",
    linkCompetitionRules: "League",
    linkPrizeRules: "Prizes",
    sectionRoundsType: "Tournament type and rounds",
    sectionRoundsTypeDesc:
      "Continue with operational parameters, rounds, and complementary rules.",
    openRounds: "Open rounds",
    linkCutRules: "Cut rules",
    noTournamentsBefore: "No tournaments available. Create one first at",
    noTournamentsPath: "/tournaments/new",
  },
  editors: {
    leaderboardColumn: "Leaderboard",
  },
  publicTournament: {
    home: "← Home",
    seeTournaments: "Browse tournaments",
    adminList: "Tournament list",
    live: "Live scoring",
    leaderboard: "Leaderboard",
    teeSheet: "Starters / Groups",
    favorites: "Favorites",
    cercanosTab: "Closest to pin",
    convocatoria: "Tournament call",
    bracket: "Bracket",
    bracketLiveTab: "Live bracket",
    matchesLiveTab: "Live matches",
    pageTitleBracket: "Match play bracket",
    pageDescBracket:
      "Live matches with points per hole (low ball and high ball).",
    bracketEmpty:
      "The bracket is not published yet. The committee must publish it from admin.",
    bracketFormat: "Format",
    bracketAllowance: "Handicap",
    bracketVs: "vs",
    bracketHoleDetail: "Hole-by-hole",
    bracketLowBall: "Low",
    bracketHighBall: "High",
    bracketPoints: "pts",
    bracketLive: "In progress",
    bracketCompleted: "Final",
    bracketBye: "BYE",
    eSignature: "✍️ E-signature",
    posterAlt: "Tournament poster",
    allCategories: "All categories",
    publicBadge: "Public tournament",
    playerOne: "player",
    playersMany: "players",
    categoryChip: "Category:",
    roundChip: "Round",
    pageTitleLive: "Live scoring",
    pageTitleOfficial: "Official results",
    pageTitleFavorites: "My favorites",
    pageTitleTeeSheet: "Starters / Groups",
    pageTitleConvocatoria: "Tournament call",
    pageDescConvocatoria:
      "Tournament rules and conditions published by the committee. Read-only document.",
    convocatoriaEmpty: "The tournament call is not published yet.",
    convocatoriaReadOnlyNote:
      "Informational document. It cannot be edited from this page.",
    convocatoriaGeneralHeading: "General information",
    convocatoriaCategoriesHeading: "Categories",
    pageDescOfficial: "Results verified by tournament administration.",
    pageDescFavorites: "Quick follow-up for players marked as favorites.",
    favoritesAllCategoriesHint:
      "Every player you mark with ★, from any category. POS, thru, and totals follow each player’s current round.",
    pageDescTeeSheet:
      "Public tee sheet by day, visible only after the committee confirms the final order.",
    pageDescLive:
      "Live results with score entry progress and positions by category.",
    statRounds: "Rounds",
    statStarts: "Starters",
    statPending: "Pending",
    cardLiveKicker: "Live scoring",
    cardLiveTitle: "Live tracking",
    cardLiveBody:
      "Publicly visible scoring without entering the admin system.",
    cardTeeKicker: "Starters / Groups",
    cardTeeTitle: "Groups by day",
    cardTeeBody:
      "Public starters appear after the committee confirms the final order for the day.",
    cardSignKicker: "E-signature",
    cardSignTitle: "Direct access to signing",
    cardSignBody:
      "Go to score entry and validation for player and marker signatures.",
    cardSignCta: "Go to e-signature",
    officialChipClosed: "closed",
    officialChipPending: "missing",
    officialChipDone: "complete",
    officialCloseRoundLabel: "Round {n} · status by category",
    officialShowPendingList: "Show {n} pending",
    officialHidePendingList: "Hide list",
    officialPendingHeading: "Not yet closed",
    officialCaptureCta: "Enter scores",
    officialPendingManyHint: "{n} players still to close",
    liveRoundToday: "Today",
    liveRoundHistory: "Past days",
    liveRoundFuture: "Upcoming dates",
    liveRoundUndated: "No calendar date",
    viewingRoundKicker: "Viewing",
    noRoundSelected: "Pick a category; the default round is today for that category.",
    historicRoundsToggle: "History: other dates & rounds",
    detailHolesCol: "HOLES",
    detailParRow: "Par",
    /** US card: holes 1–9, OUT (front nine total), 10–18, IN (back nine total), then GR / vs / POS. */
    detailFirstNineTitle: "OUT",
    detailFirstNineSub: "",
    detailSecondNineTitle: "IN",
    detailSecondNineSub: "",
    detailTotalTitle: "",
    detailTotalSub: "",
    detailTotalsPlacement: "inline-after-nines" as
      | "inline-after-nines"
      | "trailing-after-18",
    detailGross: "GR",
    detailToPar: "vs",
    detailPos: "POS",
    detailNoCapture: "No scores for the selected round.",
    detailExpandedCategoryLabel: "Category",
    detailExpandedClubLabel: "Club",
    detailCloseButton: "Close",
    detailCloseAria: "Close scorecard detail",
    detailToggleOpenAria: "Show scorecard detail",
    detailToggleCloseAria: "Hide scorecard detail",
    leaderboardEmptyView: "No players to show in this view.",
    playerSearchPlaceholder: "Search player, club, or category…",
    playerSearchAria: "Filter the table by name, club, or category",
    playerSearchHint:
      "Filters the current view. If you do not know the category, pick \"All categories\" above and search here.",
    playerSearchNoMatches: "No player matches your search.",
    playerSearchCount: "{shown} of {total}",
    detailAuditStrokeIndex: "SI",
    detailAuditStrokesReceived: "HCP",
    detailAuditNetStrokes: "Net",
    detailAuditStablefordPoints: "Pts",
    categoryCompetitionTitle: "Competition",
    categoryCompetitionModality: "Format",
    categoryCompetitionLeaderboard: "Leaderboard",
    categoryCompetitionPrizes: "Prizes",
    categoryCompetitionGrossPlaces: "{n} gross places",
    categoryCompetitionNetPlaces: "{n} net places",
    categoryCompetitionStablefordPlaces: "Stableford prizes",
    categoryCompetitionGrossNetHint:
      "Use the Gross / Net tabs to switch the displayed standings.",
    basisToggleGross: "Gross",
    basisToggleNet: "Net",
    basisToggleAria: "Switch gross or net leaderboard",
    prizesPanelTitle: "Prizes",
    prizesConfiguredPlaces: "Competition places",
    prizesGrossPlace: "{n} gross",
    prizesNetPlace: "{n} net",
    prizesStablefordPlace: "{n} Stableford",
    prizesNoDetailedRules: "No detailed prize rules published for this category.",
    prizesBasisGross: "Gross",
    prizesBasisNet: "Net",
    prizesBasisStableford: "Stableford",
    loadErrorTitle: "Could not load tournament",
    loadErrorBody:
      "An error occurred while loading tournament data. Please try again shortly.",
    loadErrorTechnical: "Technical detail",
    leaderboardBuildErrorTitle: "Leaderboard calculation failed",
    leaderboardBuildErrorBody:
      "Tournament data loaded, but ranking could not be built. Check admin configuration.",
    rulesBlockedTitle: "Leaderboard unavailable",
    rulesBlockedIntro:
      "Tournament configuration must be fixed in admin before results can be shown.",
    rulesServiceRoleNotConfigured:
      "SUPABASE_SERVICE_ROLE_KEY is missing on the server (Vercel → Environment Variables). Competition rules cannot be loaded without it.",
    rulesCompetitionLoadFailed:
      "Competition rules could not be loaded from the database.",
    rulesCutLoadFailed:
      "Cut rules could not be loaded (required from round 2 onward).",
    rulesCategoriesMissingRule:
      "Missing active competition rule for: {codes}.",
    rulesCompetitionInvalidConfig:
      "Invalid competition rules: {details}.",
    rulesStrokeIndexIncomplete:
      "Course holes incomplete: only {count} of 18 have stroke index (required for net/Stableford).",
    rulesBlockedAdminHint:
      "Admin → Competition, Cut rules, and Tournament holes.",
  },
  publicTeeSheet: {
    empty:
      "No published tee times yet. The committee must confirm/close the final order for the day in Starters / Groups.",
    allDays: "All days",
    noGroupsFilter: "No groups to show with this filter.",
    publishedStarts: "Published tee times",
    groupOne: "group",
    groupMany: "groups",
    startingTee: "Tee",
    playerOne: "player",
    playersMany: "players",
    scoreHcp: "HCP",
    scoreR1: "R1",
    scoreR1R2: "R1+R2",
  },
  publicInstall: {
    rowLabel: "Shortcut",
    button: "App icon",
    aria: "How to add a listgolf.club shortcut",
    title: "Quick access to List.golf",
    intro:
      "Add an icon to open the tournament in one tap. Steps differ for iPhone, Android, and Mac.",
    installNow: "Install on this device",
    stepsLead: "Follow these steps (with pictures):",
    otherDevices: "Other platforms",
    close: "Got it",
    highlightShare: "Share",
    highlightMenu: "Menu ⋮",
    ios: {
      title: "iPhone or iPad",
      note: "Use Safari. Chrome on iPhone does not offer “Add to Home Screen”.",
      steps: [
        {
          text: "Open listgolf.club in Safari (not inside WhatsApp or Instagram).",
          visual: "ios-share",
        },
        {
          text: "At the bottom, tap Share (square with up arrow ↑). This only opens a menu.",
          visual: "ios-share",
          highlight: "Share",
        },
        {
          text: "Scroll the menu and tap “Add to Home Screen” (not Messages or Mail).",
          visual: "ios-add-home",
        },
        {
          text: "Tap “Add” at the top right. Without this, the icon is not saved.",
          visual: "ios-confirm",
        },
      ],
    },
    android: {
      title: "Android",
      note: "Chrome or a similar browser.",
      steps: [
        {
          text: "Tap the three-dot menu ⋮ at the top right.",
          visual: "android-menu",
          highlight: "Menu ⋮",
        },
        {
          text: "Choose “Add to Home screen” or “Install app” and confirm.",
          visual: "android-install",
        },
      ],
    },
    macSafari: {
      title: "Mac with Safari",
      note: "There is no “Add to Home Screen” like on iPhone. In Safari use “Add to Dock” (icon in the bottom bar).",
      steps: [
        {
          text: "Open listgolf.club in Safari (not Chrome).",
          visual: "mac-file-dock",
        },
        {
          text: "Top menu bar: File → “Add to Dock…”",
          visual: "mac-file-dock",
        },
        {
          text: "Confirm in the dialog. The icon appears in your Mac Dock.",
          visual: "mac-dock-result",
        },
      ],
    },
    macChrome: {
      title: "Mac with Chrome",
      note: "Install as a web app or create a Dock shortcut.",
      steps: [
        {
          text: "Look for the ⊕ Install control in the address bar.",
          visual: "chrome-install",
        },
        {
          text: "Or ⋮ menu → “Install List.golf…” / “Create shortcut”.",
          visual: "chrome-install",
        },
      ],
    },
    macOther: {
      title: "Mac (other browser)",
      note: "Try Safari (File → Add to Dock) or Chrome (Install).",
      steps: [
        {
          text: "Recommended: open the site in Safari and use File → Add to Dock.",
          visual: "mac-file-dock",
        },
        {
          text: "Or save a bookmark to the favorites bar for quick access.",
          visual: "bookmark-bar",
        },
      ],
    },
    desktopChrome: {
      title: "Computer (Chrome / Edge)",
      note: "Install as an app or shortcut.",
      steps: [
        {
          text: "Click Install or the ⊕ icon in the address bar.",
          visual: "chrome-install",
        },
        {
          text: "Or browser menu → Install / Create shortcut.",
          visual: "chrome-install",
        },
      ],
    },
    desktopOther: {
      title: "Computer",
      note: "Use a bookmark or try Chrome to install the app.",
      steps: [
        {
          text: "Drag the URL to the bookmarks bar for quick access.",
          visual: "bookmark-bar",
        },
      ],
    },
  },
  capturaTelegram: {
    title: "Telegram mode",
    subtitle:
      "Mobile capture links by group. Share them via Telegram to players and caddies.",
    pickTournament: "Tournament",
    pickRound: "Round",
    noTournament: "Pick a tournament to see groups.",
    noRound: "This tournament has no rounds yet. Create a round first.",
    noGroups: "The selected round has no groups yet. Build groups in Tee Sheet.",
    table: {
      group: "Group",
      hole: "Hole",
      teeTime: "Time",
      players: "Players",
      caddies: "Caddies",
      link: "Capture link",
      actions: "Actions",
      noCaddies: "No caddies",
    },
    btnCopy: "Copy",
    btnCopied: "Copied!",
    btnQr: "QR",
    btnSend: "Send Telegram",
    btnSending: "Sending…",
    btnSendAll: "Send to WHOLE tournament",
    btnSendAllConfirm: "Send the link to every group?",
    qrModalTitle: "Group QR",
    qrModalHint:
      "The caddie or player can scan this QR to open the capture link.",
    qrModalClose: "Close",
    sendResultSuccess: "Sent to {sent} recipient(s).",
    sendResultPartial: "Sent to {sent} · {failed} not linked to Telegram.",
    sendResultEmpty: "Nobody has Telegram linked.",
    sendResultError: "Error sending via Telegram.",
    linkedYes: "TG",
    linkedNo: "—",
    legendTitle: "How it works",
    legendBody:
      "Players link from Entries → KIT. Caddies link by writing «HOLA» to the tournament bot. Only linked recipients can receive the link automatically.",
    statTitle: "Tournament status",
    statPlayersLinked: "Players linked",
    statCaddiesLinked: "Caddies linked",
    statGroups: "Groups",
    msgTitle: "📋 Group scorecard capture",
    msgIntro:
      "Here's the link to the group scorecard. Anyone in the group (player or caddie) can capture and everyone will see changes in real time.",
  },
  handicapCommittee: {
    pageTitle: "Handicap Committee",
    pageSubtitle: "Anonymous vote · Tournament handicap adjustment (−1 to −5 strokes)",
    notLoggedIn: "Sign in to access the committee.",
    noTournament: "Select a tournament to use the handicap committee.",
    pickTournament:
      "Pick a tournament to audit handicaps or vote (anonymous vote among members).",
    pickOpenVote: "Choose which committee vote to enter.",
    changeTournament: "Change tournament",
    noOpenVote: "There is no open committee vote at this time.",
    noDailyRounds:
      "The handicap committee applies to tournaments only, not daily rounds.",
    noTournaments:
      "You don't have tournaments assigned. Ask an administrator to grant you the «Handicap Committee» or «Tournament Director» role in a tournament.",
    noAccess: "You don't have access to this tournament's handicap committee.",
    bannerEnabled:
      "Committee enabled. Assign the «Handicap Committee» role to members in Users.",
    bannerClosed: "Voting closed.",
    bannerHiApplied: "Tournament handicap updated with the committee adjustment.",
    bannerHiAppliedBulk: "Tournament handicap updated for {n} player(s).",
    bannerHiAppliedBulkPartial:
      "Tournament handicap updated for {ok} player(s). {fail} failed.",
    bannerPresent: "Member marked as present.",
    bannerAbsent: "Member marked as absent.",
    bannerRoleAssigned: "User added to the committee.",
    bannerRoleRevoked: "User removed from the committee.",
    bannerTrimSaved: "Outlier trim updated.",
    bannerVotesReset:
      "Voting reset: the previous session was archived and all members must vote again from scratch.",
    adminBadge: "Admin only",
    adminScopeDesc:
      "As Director / Club Admin / Super Admin you can set up rules, mark attendance, close voting, reset and view history.",
    btnVote: "Vote",
    btnAdmin: "⚙ Administration",
    btnManageMembers: "Manage members",
    notActive:
      "The committee is not active in this tournament yet. An administrator must enable it first.",
    emptyNoEntries:
      "No active entries in this tournament yet. Add players in {entriesLink} so the committee can review them.",
    entriesLinkLabel: "Entries",
    emptyNoFlagged:
      "No players on the ballot yet. In {selectionLink} mark who the committee should review: marking one adds them instantly for every member.",
    emptyNoFlaggedVoter:
      "No players on the ballot yet. Any member can mark them in {selectionLink}; marking one adds them for everyone. There is no per-person list.",
    selectionLinkLabel: "Player selection",
    voter: {
      presenceTitle: "You are not marked as present.",
      presenceAdmin:
        "Mark your attendance in the «Committee members» section of the Administration panel to vote.",
      presenceMember:
        "Ask the tournament director to enable you in the committee panel. Until then you cannot save votes.",
      notCommitteeTitle: "You can view the committee, but you cannot vote.",
      notCommitteeBody:
        "Only users with the handicap_committee role may cast a vote. A present director, marshal, or club admin does not move handicaps.",
      progressLabel: "Your progress",
      progressClosed:
        "Voting is closed. You can no longer change your ratings.",
      progressVotePrivate:
        "Private vote. Lower tournament handicap only (−1 to −5 strokes).",
      tabPending: "Pending",
      tabRated: "Rated",
      tabResults: "Results",
      searchPlaceholder: "Search player, club, HP…",
      clearSearch: "Clear search",
      pendingMissing: "Missing",
      pendingFiltered: " (filtered)",
      pendingRated: "Rated",
      ratedShowing: "Showing",
      ratedSuffix: "rated",
      ratedRemaining: "Missing",
      ratedOf: "of",
      emptyPendingSearch: "No pending players match the search.",
      emptyPendingDone: "Done! No players left to rate.",
      emptyRatedSearch: "No ratings match your search.",
      emptyRatedNone: "You have not rated any player yet.",
      closedHint:
        "Voting is closed. Switch to «Results» to see averages or «Rated» to review your votes.",
      resultsEmpty: "No aggregated results to show yet.",
      resultsLiveTitle: "Live partial results",
      resultsFinalTitle: "Final results",
      resultsLiveBadge: "🔴 Live",
      resultsFinalBadge: "✓ Final",
      resultsHelp:
        "Tap the player name (blue underline) to see the vote-by-vote distribution and abstentions. Anonymous.",
      thPlayer: "Player",
      thHi: "HP",
      thHiTitle: "Tournament playing handicap (strokes)",
      thVotes: "Votes",
      thVotesTitle: "Votes cast (adjustments + abstentions)",
      thAdj: "Adj.",
      thAdjTitle: "Numeric adjustments that survive the trim",
      thAbst: "Abst.",
      thAbstTitle: "Abstentions (not adjustments)",
      thAvg: "Avg",
      thAvgTitle: "Trimmed average of the adjustment (HP strokes)",
      thHiSug: "Sug HP",
      thHiSugTitle: "Suggested HP = tournament handicap + average in strokes",
      thVetos: "Vetoes",
      thVetosTitle: "Votes of «Do not allow to play this tournament»",
      legendHi: "Current tournament handicap",
      legendVotes: "total cast (adjustments + abstentions)",
      legendAdj: "numeric votes that survive the trim",
      legendAbstOut: "do not enter the average",
      legendAbstInAvg: "count as 0 in the denominator",
      legendAvg: "trimmed average (HP strokes)",
      legendHiSug: "HP + Avg",
      legendVetos: "«Do not allow to play» votes",
      expandedTitle: "Vote by vote (anonymous)",
      expandedNoVotes: "No votes for this player yet.",
      expandedHi: "Current HP",
      expandedAvg: "Avg.",
      expandedHiSug: "Sug HP.",
      chipAbstainOut: "Abstention (not in the average)",
      chipAbstainInAvg: "Abstention (counts as 0 in the average)",
      chipTrimmedLow: "Discarded (most severe)",
      chipTrimmedHigh: "Discarded (most lenient)",
      chipLive: "Live vote",
      chipAbst: "0·abst",
    },
    card: {
      ghinTitle: "Player GHIN",
      ghinChip: "GHIN",
      reviewBadge: "Review",
      hiLabel: "HP",
      chLabel: "CH",
      chTitlePrefix: "Slope",
      chTitleFallback: "Course Handicap",
      phLabel: "PH",
      phTitlePrefix: "Playing Handicap (allowance",
      phTitleFallback: "Playing Handicap",
      savedAria: "Vote saved",
      savedJustNow: "Saved",
      pendBadge: "Pend",
      voteOpenReport: "📄 View GHIN report",
      youAbstained: "You abstained",
      onThisPlayer: "on this player.",
      ratingSavedPrefix: "Rating saved:",
      pts: "strokes",
      dqMarkedLine: "You marked: do not allow to play this tournament",
      lockedHint: "Locked · voting closed",
      editBtn: "Edit rating",
      abstainBtn: "No opinion (abstain)",
      lowerHiLabel: "Lower tournament handicap",
      sliderMin: "−5 (max)",
      sliderMax: "−1 (min)",
      dqTitle: "Do not allow to play this tournament",
      dqHint:
        "Only in exceptional cases (misconduct, cheating, serious risk). Anonymous vote.",
      dqBadgeMarked: "Marked",
      dqBadgeTap: "Tap",
      saveBtn: "Save rating",
      saveBtnUpdate: "Save changes",
      cancelBtn: "Cancel",
      errorSave: "Save error",
      tooLateClosed: "You did not vote in time and voting is now closed.",
      summaryHiCurrent: "Current HP",
      summaryAvg: "Average",
      summaryHiSug: "Suggested HP",
      summaryLiveLong: "adj.",
      summaryNumShort: "num.",
      summaryAbstSuffix: "abst.",
    },
    admin: {
      sectionTitle: "Committee administration",
      expectedMembers: "Expected members",
      activate: "Enable committee in this tournament",
      statusLabel: "Status:",
      statusOpen: "Open",
      statusClosed: "Closed",
      membersWithRole: "Members with role:",
      expectedMembersHint:
        "Expected quorum indicator. It does not enter the trim or the disqualification threshold.",
      presentToday: "Present today:",
      canCloseHint:
        "You can close voting even if some members have not rated every player.",
      btnClose: "Close voting",
      btnReopen: "Reopen voting",
      membersTitle: "Club committee members",
      membersHelp:
        "Define once who the permanent committee members are (scope «Whole club»). For each tournament you just mark which of them are attending to enable their vote in that session.",
      attendanceTitle: "Attendance for this tournament",
      attendanceHint:
        "Use «✅ Mark present» on the members who will attend the vote for THIS tournament. The rest stay inactive this session but keep their club role.",
      addMember: "Add member to club committee",
      chooseUser: "— pick a user —",
      scopeLabel: "Scope",
      scopeTournament: "This tournament only",
      scopeClub: "Whole club",
      scopeGlobal: "Whole system",
      scopeHint:
        "Recommended: «Whole club» so the member becomes a permanent committee member.",
      btnAddCommittee: "+ Add to committee",
      addCommitteeHint:
        "The user will get the «Handicap Committee» permission for the chosen scope. Then, on each tournament, mark their attendance with the green button on their card to enable their vote in that session.",
      inviteByEmail: "Not in the list? Invite by email",
      userEmail: "User email",
      emailPh: "member@email.com",
      btnSearchAuthorize: "Search and authorize",
      inviteRequires:
        "Requires the user to already exist in the system. If they don't, create them first in",
      usersNewLabel: "Users → New",
      noMembersWarning:
        "There are no authorized members yet. Use the form above or assign the role in",
      usersLink: "Users",
      directorsAutoIncluded:
        ". Tournament directors are automatically included.",
      globalProgress: "Committee global progress",
      votesShort: "votes",
      activeMembers: "Active members:",
      invalidVotesTitle: "Invalid votes (not included in the average)",
      invalidVotesHint:
        "Kept in the database. Only handicap_committee members marked present are counted.",
      invalidNoRole: "no committee role",
      invalidNotPresent: "not present",
      playersLabel: "Players:",
      completed100: "Completed 100%:",
      pendingMembers: "Pending:",
      scopeBadgeTournament: "This tournament",
      scopeBadgeClub: "Whole club",
      scopeBadgeGlobal: "Whole system",
      director: "Director",
      present: "Present",
      absent: "Absent",
      votesDone: "Votes cast",
      abstSuffix: "abst.",
      btnMarkAbsent: "🔕 Mark absent",
      btnMarkPresent: "✅ Mark present",
      btnRevoke: "Remove",
      trimSectionTitle: "Outlier trim and «Do not play» threshold",
      removeHighVotes: "Remove high votes (most lenient)",
      removeLowVotes: "Remove low votes (most severe)",
      minDqVotes: "Minimum «Do not play» votes",
      btnSaveRules: "Save rules",
      trimExplain:
        "Trim: for each player the N votes closest to 0 and the N votes closest to −5 are discarded; the average and suggested HP are recalculated using the live votes. Trim is not reduced when there are few votes: if none survive, the adjustment stays at 0.",
      thresholdExplain1:
        "«Do not play» threshold: if a player accumulates that number (or more) of «Do not allow to play this tournament» votes, they are highlighted in red below.",
      thresholdExplain2: "= only the count is shown, without highlighting.",
      abstentionsInAverage: "Count abstentions as 0 in the average",
      abstentionsInAverageHint:
        "Off (recommended): average uses only numeric adjustments that survive the trim. On: abstentions dilute the average toward 0.",
      aggregateHelp:
        "Aggregate summary (anonymized): individual votes are shown without names. Green = adjustment that enters the average, gray = discarded by trim. Adjustments = numeric votes that survive the trim (enter the average). Abst. = abstentions (enter the average only if the committee option is on). A player with only abstentions shows 0 adjustments.",
      thPlayer: "Player",
      thHiCurrent: "Tournament HP",
      thVotesAnon: "Votes (anonymous)",
      thLiveAvg: "Adj. / Abst.",
      thLiveAvgTitle:
        "Adj. = numeric votes that enter the average (after trim). Abst. = abstentions. The average denominator follows the committee option.",
      thAdjShort: "Adj.",
      thAbstShort: "Abst.",
      thAdjTitle: "Numeric adjustments that survive the trim (enter the average)",
      thAbstTitle: "Abstentions (not adjustments; enter the average only if enabled)",
      thAvgTrim: "Trimmed avg.",
      thHiSug: "Suggested HP",
      thNoPlay: "Do not play",
      thRoundedAdj: "HP adj.",
      thRoundedAdjTitle: "Stroke adjustment to tournament handicap (integers, −5 to 0)",
      thFinalHi: "Final HP",
      thFinalHiTitle: "Tournament handicap that will be applied (current HP + adjustment)",
      applyHi: "Apply tournament handicap",
      applyHiDone: "Already applied",
      committeeAppliedNote: "Tournament handicap already applied (committee: {adj})",
      committeeAppliedHint: "Change the adjustment to apply again",
      applyAllSelected: "Apply to selected",
      applyAllBtn: "Apply tournament handicap to all",
      bulkBarTitle: "Apply tournament handicap",
      bulkBarHint:
        "Adjustments are strokes applied directly to tournament handicap (HP). With no votes the adjustment starts at 0 and you can edit it before applying.",
      bulkBarSelected: "selected",
      bulkBarApplying: "Applying…",
      bulkBarSelectAll: "Select all",
      bulkBarClear: "Clear all",
      manualAdjNoVotesHint:
        "No committee votes: manual adjustment (starts at 0).",
      noVotesYet: "—",
      noLiveVotes: "No live votes",
      ghinTitle: "Player GHIN",
      noVotes: "No votes",
      avgDivisor: "Avg.: ÷",
      abstAsZero: "abst. in average",
      chipAbstain: "Abstention (metadata; does not dilute average unless enabled)",
      chipTrimmedLow: "Discarded (most severe)",
      chipTrimmedHigh: "Discarded (most lenient)",
      chipLive: "Live vote",
      chipAbst: "0·abst",
      thresholdAuto: "Exceeds threshold: player not authorized",
      thresholdConfigured: "Configured threshold:",
      thresholdInfo: "Informational only (threshold disabled)",
      votesWord: "votes",
      notAuthorized: "Not authorized",
    },
    reset: {
      toggle: "Reset voting (test)",
      sessionName: "Name of this session (optional)",
      sessionNamePh: "e.g. First vote",
      notes: "Notes (optional)",
      notesPh: "Reason for reset",
      confirmLabel: "Type RESET to confirm",
      confirmPh: "RESET",
      submit: "Archive and clear votes",
      archiving: "Archiving…",
      successArchived:
        "Session «{name}» archived. {n} players with votes, {m} votes deleted.",
      noVotes:
        "There are no votes to archive. Voting is already empty; no new session was created.",
      hint: "Saves an anonymous summary of the current vote to history and clears every member's votes. Each member must rate all players again.",
    },
    history: {
      title: "Voting history",
      titleWithCount: "Voting history ({n})",
      empty:
        "No archived sessions yet. When voting is reset, an anonymous summary will be saved here.",
      subtitle:
        "Sessions saved on reset. Individual votes remain anonymous.",
      sessionFallback: "Session",
      summaryVoters: "voters",
      summaryPlayers: "players",
      trimSummary: "Trim: −{soft} lenient / −{hard} severe",
      vetoThreshold: "«Do not play» threshold:",
      thresholdOff: "off",
      presentLabel: "Present:",
      notesLabel: "Notes:",
      thPlayer: "Player",
      thHi: "HP",
      thVotes: "Votes",
      thAvg: "Avg.",
      thHiSug: "Sug HP",
      thNoPlay: "Do not play",
      chipAbstention: "Abstention",
      chipTrimmed: "Trimmed out",
      chipLive: "Live vote",
      chipAbst: "0·abst",
      notAuthorized: " · Not authorized",
    },
  },
};

export const messages: Record<Locale, Messages> = { es, en };

export type AppMessages = Messages;
