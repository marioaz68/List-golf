...
3. Handicap de torneo "oficial 80%" (columnas de inscritos y seleccion del comite): `HP = redondeo(CH_exacto x 0.80)`, con el % sobre el CH DECIMAL — lib/ghin-report/handicapMath.ts:54 y lib/handicap/resolveTournamentEntryHandicap.ts:398. Validacion del comite: HI 25.6 en Blancas (70.7/127/72) -> CH_exacto 27.47 -> CH 27 -> HP 22 (lib/ghin-report/loadGhinReport.ts:844).
4. Existen por tanto DOS formulas de PH en el repo (reglas 2 y 3) que pueden diferir 1 golpe cuando CH_exacto cae en .5 (ej. CH_exacto 20.5 -> regla 2 da 17, regla 3 da 16). `resolveOfficialHcp80` fuerza 80% aunque la categoria tenga otro % — lib/handicap/resolveTournamentEntryHandicap.ts:412.
...
- Los dos redondeos de PH (reglas 2 y 3) no son intercambiables. `resolveOfficialHcp80` aplica el % al CH decimal; `computeWhsHandicap` al CH entero. Unificar sin pensarlo mueve un golpe a decenas de jugadores (comentario en lib/ghin-report/handicapMath.ts:8).
